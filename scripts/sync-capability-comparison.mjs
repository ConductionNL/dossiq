 ...
 *
 * Re-issue `src/data/capabilityComparison.json` from the parity corpus.
 *
 ...
 * WHAT IT WRITES
 * --------------
 *   src/data/capabilityComparison.json           the page's data
...
const DATA = join(HERE, '..', 'src', 'data', 'capabilityComparison.json')
...
const COMMENT = [
	'Capability comparison shown on the Features & roadmap page. RE-ISSUED FROM THE CORPUS',
	'by scripts/sync-capability-comparison.mjs. Do not edit this file by hand: the corpus is',
	'ConductionNL/market-intelligence, procest/_ledger/parity-ledger.html, and every id,',
	'capability text and rating here comes from it. A hand edit is what happened on 2026-09-10,',
...
function build(corpus, current, on) {
	...
	const ids = {
		_comment:
			...
			+ 'capabilityComparison.json holds a different set. Do not edit either '
...
if (process.argv.includes('--check')) {
	...
	if (readFileSync(DATA, 'utf8') !== dataText) {
		drift.push('src/data/capabilityComparison.json')
	}
