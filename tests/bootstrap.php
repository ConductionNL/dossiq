		...
		if (dossiq_nc_root_is_installed($dossiqNcRoot) === true) {
			try {
				...
				fwrite(
					STDERR,
					sprintf(
						"[dossiq/tests/bootstrap] Nextcloud root at %s could not be initialised (%s).\n"
						. "  A half-booted server cannot be undone, so the run stops here rather than pretending to be pure-unit.\n"
						. "  Fix the instance, or run with a config that defines OC_CONSOLE (phpunit.xml does) for pure-unit mode.\n",
						$dossiqNcRoot,
						$e->getMessage()
					)
				);
