 ...
 *
 * @spec openspec/changes/archive/2026-06-23-procest-stuf-zkn-outbound-gateway/specs/stuf-zkn-outbound/spec.md#requirement-circuit-breaker-and-retry
 */
...
class CircuitBreakerServiceTest extends TestCase {
	...
	public function testCircuitResetsAfterCooldown(): void {
		...
		// Force the open timestamp into the past beyond the cooldown. The
		// open-key is the hashed form (see CircuitBreakerService::openKey()).
		$openKey = 'stuf.cb.o.' . substr(sha1('ep-4'), 0, 32);
		$this->store[$openKey] = (time() - (CircuitBreakerService::COOLDOWN_SECONDS + 1));
		$this->assertTrue($this->service->checkEndpoint($endpoint));
