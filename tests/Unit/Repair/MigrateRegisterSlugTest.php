 ...
 *
 * @covers \OCA\Dossiq\Repair\MigrateRegisterSlug
 * @covers \OCA\Dossiq\Repair\MigrateRegisterSlugDecisions
 *
...
final class MigrateRegisterSlugTest extends TestCase {
	...
	public function testRenamesARegisterThatIsPresent(): void {
		...
		self::assertSame([['dossiq', 'procest']], $written);
	...
	public function testSlugsToReadCoversBothSidesOfTheMap(): void {
		...
		self::assertContains('procest', $slugs);
		self::assertContains('dossiq', $slugs);
		self::assertContains('procest-default', $slugs);
		self::assertContains('dossiq-default', $slugs);
	...
	public function testSlugsFromToleratesMissingAndNullSlugs(): void {
		self::assertSame(
			['procest', '', ''],
			$this->decisions->slugsFrom(rows: [['slug' => 'procest'], ['slug' => null], []])
		);
	...
	public function testAConfigWriteFailureIsLoggedNotThrown(): void {
		$cfg = $this->createMock(IAppConfig::class);
		$cfg->method('getValueString')->willReturn('procest');
