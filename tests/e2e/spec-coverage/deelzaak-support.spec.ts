...
async function openCasesListOrSkip(page) {
	...
	const row = page.locator('.viewTableRow, tr[role="row"], .list-item, table tbody tr').first()
	if (await row.count() === 0) {
		test.skip(true, 'No cases in the deployed/seeded register — case list is data-dependent.')
		return false
	}
...
async function openSubCasesTabOrSkip(page) {
	...
	await page.waitForTimeout(1000)
	const subCasesTab = page.getByRole('tab', { name: /Sub-cases|Deelzaken/i }).first()
...
test.describe('Sub-case count badge (deelzaak-support REQ — case list)', () => {
	...
	test.fixme('the case list renders and may show an "N deelzaken" badge in a single batch', async ({ page }) => {
		...
		page.on('request', (req) => {
			if (req.url().includes('/api/deelzaken/counts')) countCalls.push(req.url())
		})
		await page.reload().catch(() => {})
		await openCasesListOrSkip(page)
		await page.waitForTimeout(1500)

		await expect(page.locator('table, .viewTable, [role="table"]').first()).toBeVisible({ timeout: 10000 })
		...
		if (await badge.count() > 0) {
			...
			test.info().annotations.push({ type: 'note', description: 'No badge present — seeded deelzaak demo not deployed (no-badge branch).' })
		}
...
test.describe('Sub-case orphan deletion (deelzaak-support REQ — deletion protection)', () => {
	...
	test('the sub-cases page delete control warns about orphans for a parent with sub-cases', async ({ page }) => {
		const opened = await openSubCasesTabOrSkip(page)
		if (!opened) return

		const deleteBtn = page.getByRole('button', { name: /Delete case|Zaak verwijderen|Delete parent case|Hoofdzaak verwijderen/i }).first()
		...
		const warning = page.getByText(/unlink the sub-cases|losgekoppeld van hun hoofdzaak/i).first()
		if (await warning.count() > 0) {
			await expect(warning).toBeVisible({ timeout: 5000 })
			const cancel = page.getByRole('button', { name: /Cancel|Annuleren/i }).first()
			if (await cancel.count() > 0) await cancel.click().catch(() => {})
		} else {
			test.info().annotations.push({ type: 'note', description: 'No orphan warning — case has no sub-cases (standard-delete branch).' })
		}
		await expect(page.locator('body')).not.toContainText('Internal Server Error')
	})
})
...
test.describe('Sub-cases list + create (deelzaak-support REQ — section / creation)', () => {
	...
	test('the Sub-cases tab renders either a list or an empty state without error', async ({ page }) => {
		const opened = await openSubCasesTabOrSkip(page)
		if (!opened) return
		...
		const empty = page.getByText(/No sub-cases yet|Nog geen deelzaken|geen deelzaken/i).first()
	...
	test('the Create sub-case control opens a filtered dialog when allowed, and is hidden otherwise', async ({ page }) => {
		const opened = await openSubCasesTabOrSkip(page)
		if (!opened) return

		const createBtn = page.getByRole('button', { name: /Create sub-case|Create first sub-case|Deelzaak aanmaken|Create Sub-case/i }).first()
		if (await createBtn.count() === 0) {
			...
			test.info().annotations.push({ type: 'note', description: 'Create sub-case button hidden — parent not eligible (closed / itself a sub-case / no subCaseTypes).' })
			await expect(page.locator('body')).not.toContainText('Internal Server Error')
			return
		}
		...
		await expect(
			page.getByText(/Sub-case type|Parent case type|No allowed sub-case types/i).first(),
		).toBeVisible({ timeout: 8000 })
		const cancel = page.getByRole('button', { name: /Cancel|Annuleren|Close/i }).first()
		if (await cancel.count() > 0) await cancel.click().catch(() => {})
	})
})
...
test.describe('Sub-case breadcrumb + roll-up (deelzaak-support REQ — navigation / progress)', () => {
	...
	test('opening a sub-case shows the parent breadcrumb and the list shows a completion roll-up', async ({ page }) => {
		const opened = await openSubCasesTabOrSkip(page)
		if (!opened) return
		...
		const rollup = page.getByText(/\(\d+\/\d+ completed\)|\(\d+\/\d+ voltooid\)/i).first()
		...
		if (await subRow.count() === 0) {
			test.info().annotations.push({ type: 'note', description: 'No sub-case rows to open — breadcrumb path not reachable on this case.' })
			return
		}
		await subRow.click().catch(() => {})
		await page.waitForTimeout(800)
		const breadcrumb = page.locator('nav[aria-label="breadcrumb"], .deelzaak-detail__breadcrumb').first()
		if (await breadcrumb.count() > 0) {
			await expect(breadcrumb).toBeVisible({ timeout: 5000 })
		} else {
			test.info().annotations.push({ type: 'note', description: 'Breadcrumb not rendered — row did not navigate to DeelzaakDetail in this deploy.' })
		}
		await expect(page.locator('body')).not.toContainText('Internal Server Error')
	})
})
