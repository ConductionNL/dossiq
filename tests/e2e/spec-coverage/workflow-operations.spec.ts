...
test.describe('Case Map page', () => {
	...
	test('case map renders its heading and an interactive map surface', async ({ page }) => {
		...
		// the view renders.
		await expect(page.getByRole('heading', { name: 'Cases on map' }))
			.toBeVisible({ timeout: 15000 })
...
test.describe('Subsidies intake page', () => {
	...
	test.fixme('subsidies index renders the subsidy intake list shell', async ({ page }) => {
		const errors = trackProcestErrors(page)
		...
		await expect(page.getByRole('button', { name: /^Add Subsidie/ })).toBeVisible()
		await expect(page.locator('body')).not.toContainText('Internal Server Error')
		expect(errors, errors.join('\n')).toEqual([])
	})
})
...
test.describe('Grant schemes page', () => {
	...
	test('grant schemes index renders its list shell', async ({ page }) => {
		...
		await expect(page.getByRole('button', { name: 'Cards' })).toBeVisible({ timeout: 15000 })
		await expect(page.getByRole('button', { name: 'Table' })).toBeVisible()
		await expect(page.getByRole('button', { name: 'Add Subsidieregeling' })).toBeVisible()
		await expect(page.locator('body')).not.toContainText('Internal Server Error')
		expect(errors, errors.join('\n')).toEqual([])
	})
})
