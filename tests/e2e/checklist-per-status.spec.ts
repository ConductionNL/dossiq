 ...
 *
 * The GUARD half used to seed a `caseTask` register object, because the
 * guard's own reader had not moved and the register was the table it actually
 * looked in. `StatusChecklist::tasksFor()` now asks the engine, so both halves
 * of this spec read and write the one store the transition writes.
 *
 ...
 *
 * IT USED TO SEED `caseTask`, and had to, because the guard's own reader had
 * not moved: `StatusChecklist::tasksFor()` searched the `task_schema` register
 * that `CreateTaskHandler` stopped writing at dossiq#2363, so a task seeded in
 * the engine was invisible to the thing under test. That reader now asks
 * `EngineTaskInbox::forCase()`, so the fixture and the product finally look in
 * the same table and this helper seeds where a real checklist task lands.
 *
