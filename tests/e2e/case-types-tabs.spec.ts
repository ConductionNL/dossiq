...
test.describe('Case-types admin — 7-tab integration shell', () => {
	...
	// measured at ~50s under the CI `php -S` server, which overruns the 60s
	// default budget intermittently.
	test.slow()
	...
	// @e2e openspec/changes/case-types-04-property-doc-decision-tabs/tasks.md#TASK-CT-13
	test('admin settings surface has an add-control for case types', async ({ page }) => {
		await page.goto(ADMIN_SETTINGS_URL)
		await expect(page.getByRole('heading', { name: 'Case Type Management' })).toBeVisible({ timeout: 15000 })
	...
	test('publish validation endpoint exists at the case-types route', async ({ page, request }) => {
		...
		await page.goto(ADMIN_SETTINGS_URL)
		const res = await request.patch('/index.php/apps/procest/api/case-types/non-existent-uuid', {
			data: { isDraft: false },
