...
/**
 * Test stub for OCA\OpenRegister\Event\ObjectDeletingEvent.
 *
 ...
 * class_exists() guard when the real class is present. Mirrors the real
 * class's public surface exactly — `getObject()`, `setErrors()`/`getErrors()`,
 * `stopPropagation()`/`isPropagationStopped()`,
 * `setModifiedData()`/`getModifiedData()` — so
 * `BewijsstukImmutabilityListenerTest` can exercise the real reject decision
 * through `handle()`.
 *
 * @category Test
...
if (class_exists(ObjectDeletingEvent::class) === false) {
	/**
	 * Stub class for ObjectDeletingEvent — used only in standalone unit
	 * tests.
	 */
	class ObjectDeletingEvent extends Event implements StoppableEventInterface {
		...
		/**
		 * Stop event propagation (used by hooks to reject the deletion).
		 *
		 * @return void
		 */
