...
const dashboardWidgets = manifest.pages
	...
	.flatMap((page) => [...(page.config?.widgets ?? []), ...(page.widgets ?? [])]
		.filter((widget) => widget && widget.id && widget.widgetKey === undefined)
		.map((widget) => ({ page: page.id, widget })))

const declared = dashboardWidgets.filter(({ widget }) => Array.isArray(widget.roles) && widget.roles.length > 0)
...
describe('the audience is declared on the widget definition', () => {
	it('declares roles on the managerial dashboards', () => {
		...
		expect(pages).toEqual(new Set(['Dashboard', 'Doorlooptijd', 'ProcessMiningDashboard', 'TermijnDashboard']))
	})
	...
	it('names one role vocabulary and not a spelling per widget', () => {
		...
		expect(roles).toEqual(new Set(['dossiq-teamleider']))
	})

	it('says why, beside every declaration', () => {
		for (const { widget } of declared) {
			expect(widget._rolesNote, `${widget.id} declares roles and does not say why`).toBeTruthy()
	...
	it('puts the audience on the definition and never on a placement', () => {
		const placements = manifest.pages
			.filter((page) => page.type === 'dashboard')
			.flatMap((page) => [...(page.config?.layout ?? []), ...(page.widgets ?? [])])
		...
		for (const placement of placements) {
			...
			expect(placement.roles, `${placement.widgetKey} carries roles on its placement`).toBeUndefined()
		}
	})
	...
	it('leaves the reader\'s own work undeclared, which the allowlist explains', () => {
		...
		const myTasks = dashboardWidgets.find(({ widget }) => widget.id === 'kpi-my-tasks').widget
...
describe('the figure behind a restricted widget is a server-side concern', () => {
	it('binds the SLA tile to a field the endpoint can withhold', () => {
		const sla = dashboardWidgets.find(({ widget }) => widget.id === 'kpi-sla-compliance').widget
