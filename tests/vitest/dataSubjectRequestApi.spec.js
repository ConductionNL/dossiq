...
describe('the AVG client', () => {
	...
	it('takes the preview from the case, naming no subject', async () => {
		axios.post.mockResolvedValue({ data: { uuid: 'p1', report: { protected: [] } } })

		const preview = await previewErasure('c1')

		expect(preview.uuid).toBe('p1')
		expect(axios.post).toHaveBeenCalledWith('/apps/dossiq/api/cases/c1/avg/erasure-preview')
	...
	it('runs the erasure on its own endpoint', async () => {
		...
		expect(axios.post).toHaveBeenCalledWith('/apps/dossiq/api/cases/c1/avg/erasure-run')
	})
	...
	it('asks for and reads the export on the same url, by verb', async () => {
		...
		expect(axios.post).toHaveBeenCalledWith('/apps/dossiq/api/cases/c1/avg/subject-export')
		expect(axios.get).toHaveBeenCalledWith('/apps/dossiq/api/cases/c1/avg/subject-export')
	...
	it('escapes the case id rather than pasting it into the path', async () => {
		...
		expect(axios.post).toHaveBeenCalledWith('/apps/dossiq/api/cases/a%2Fb%20c/avg/erasure-preview')
	})

	it('serves the file from the platform, not from dossiq', () => {
		expect(subjectExportDownloadUrl('e1'))
			.toBe('/apps/openregister/api/gdpr/subject-exports/e1/download')
	})

	it('keeps the server rule and sentence on a refusal', () => {
		const refusal = refusalOf({
			response: { data: { error: 'erasure-preview-stale', message: 'Take the preview again.' } },
		})
