...
describe('shapeMarkerFeatures', () => {
	it('maps OR point rows to GeoJSON Point features ([lng, lat] order)', async () => {
		const { shapeMarkerFeatures } = await import('../../src/services/mapFormatters.js')
		...
		expect(features[0].geometry).toEqual({ type: 'Point', coordinates: [5.2, 52.1] })
	...
	it('carries a status colour + icon so markers are colour-coded', async () => {
		const { shapeMarkerFeatures } = await import('../../src/services/mapFormatters.js')
		const [blocked] = shapeMarkerFeatures([{ id: 'b', lat: 1, lng: 2, status: 'blocked' }])
...
describe('fetchCasePoints — consumes the OR maps-overview points endpoint', () => {
	...
	it('hits the OR points URL and unwraps the { points } envelope', async () => {
		const { fetchCasePoints } = await import('../../src/services/casesOnMapApi.js')
		axios.get.mockResolvedValue({ data: { points: [{ id: 'c1', lat: 52, lng: 5 }], count: 1 } })

		const points = await fetchCasePoints({ register: 'procest', schema: 'case', filters: { status: 'open' } })

		expect(axios.get).toHaveBeenCalledTimes(1)
		const [url, options] = axios.get.mock.calls[0]
		expect(url).toBe('/index.php/apps/openregister/api/integrations/maps/overviews/procest/case/points')
...
describe('registerCasesOnMapOverview — declares the overview with OR', () => {
	...
	it('POSTs the overview declaration with the stable key + scope', async () => {
		const { registerCasesOnMapOverview, CASES_ON_MAP_KEY } = await import('../../src/services/casesOnMapApi.js')
		axios.post.mockResolvedValue({ data: { id: 'maps-overview:' + CASES_ON_MAP_KEY } })

		await registerCasesOnMapOverview({ register: 'procest', schema: 'case' })

		expect(axios.post).toHaveBeenCalledTimes(1)
		const [url, body] = axios.post.mock.calls[0]
		expect(url).toBe('/index.php/apps/openregister/api/integrations/maps/overviews')
	...
	it('returns null (never throws) when the declaration fails', async () => {
		const { registerCasesOnMapOverview } = await import('../../src/services/casesOnMapApi.js')
