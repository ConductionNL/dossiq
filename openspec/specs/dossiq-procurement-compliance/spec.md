---
status: done
status-note: "Synced 2026-06-14 from archived consolidation change add-dossiq-procurement-suite (kind:config). SPEC-COMPLETE; code chain pending (ADR-032). T15 polish item for the code chain: add an explicit lib/Db mapper-scan no-parallel-storage scenario for parity with the other suite specs (intent already asserted via OR-backed register declarations + service anti-pattern guards)."
---

# dossiq-procurement-compliance Specification

## Purpose
Define procurement compliance for dossiq: drempelbedragen as a
`ProcurementThreshold` register, declarative procedure-type
recommendation, UEA/EML as registers (not PDF blobs), declarative
compliance KPI widgets + notifications — anchored in Aw 2012 / EU
threshold regulations, all data-driven per ADR-031.
## Requirements
...
- **THEN** new tender procedure recommendations MUST consult the new
  thresholds without a dossiq code change.
...
Dossiq MUST NOT author `ProcurementProcedureService::recommend()` —
...
The EML-bestand SHALL be a declarative export derived from the `UeaDeclaration` register; dossiq MUST NOT author an export service.
...
dossiq MUST expose an EML-bestand XML export derived from the
...
- **THEN** the resulting XML MUST contain the structured field
  payload; the dossiq call path MUST contain no XML-templating PHP.
...
- `maverick-spend` — `[future]` integration with financeq: contracts
  in effect without a dossiq source tender, where applicable.

Dossiq MUST NOT author `ComplianceReportService` —
...
Dossiq MUST NOT author `ComplianceNotificationService`.
...
### Requirement: REQ-PCC-007 — Compliance pages SHALL be reachable through the dossiq manifest navigation
