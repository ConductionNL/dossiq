---
status: done
status-note: "Synced 2026-06-14 from archived consolidation change add-dossiq-procurement-suite (kind:config). SPEC-COMPLETE; code chain pending (ADR-032). Note: a Supplier Contract schema already exists on development (leverancier-zaakportaal) — the CLM code chain MUST reconcile the canonical Contract additively against it, not create a parallel register."
---

# dossiq-procurement-contract-lifecycle Specification

## Purpose
Define contract lifecycle management for dossiq: contracts as an
OpenRegister `Contract` register governed by a `contract-lifecycle`
caseType, with declarative lifecycle, e-signature via OpenConnector,
docudesk-owned documents, declarative notifications + analytics — reusing
dossiq's workflow-engine + parafering abstractions (ADR-022/031).
## Requirements
...
`lib/Settings/dossiq_register.json` per ADR-024, with the `Contract`
...
| `parafeerrouteId` | string | No | FK to a dossiq `parafeerroute` for the signature endorsement chain |
...
Dossiq MUST seed a `caseType` named `contract-lifecycle` (Schema.org
...
termination) play out using dossiq's existing
...
`lib/Settings/dossiq_register.json` seeds — not as PHP). Operators
...
Per ADR-031, dossiq MUST NOT author `ContractService::transition*`
...
Dossiq MUST NOT author a `DocuSignClient`, `ValidSignService`, or
any signature HTTP wrapper — that is the ADR-019 anti-pattern.

The signing-package itself uses dossiq's existing
...
- **GIVEN** the dossiq codebase post-implementation
...
Dossiq MUST NOT define a `lib/Service/ContractDocumentService.php`
...
- **THEN** the document MUST be fetched from `docudesk` via the URI in
  `signedDocumentRef`; dossiq's code path MUST NOT contain the PDF
  bytes.
...
Dossiq MUST NOT author `ContractNotificationService` — per ADR-031
...
Contract analytics SHALL be derived via declarative aggregations and widgets; dossiq MUST NOT author an analytics service.
...
Dossiq MUST NOT author `ContractAnalyticsService` or
`ContractStatsService`.

The widgets are consumed by dossiq's existing dashboard
...
### Requirement: REQ-CLM-008 — Contract registers SHALL be reachable through the dossiq manifest navigation
...
- **WHEN** a contract manager opens
  `/index.php/apps/dossiq/contracts/renewals`
- **THEN** the page MUST render via `CnIndexPage` showing only
  contracts whose state matches the filter — no dossiq-side filter
