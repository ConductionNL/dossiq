...
# dossiq-tenant-migration Specification

## Purpose

Migrate dossiq's parallel `TenantService`, `TenantMiddleware`, and `TenantController` to
...
- AND dossiq's TenantMiddleware resolves this group to OR Organisation `org-123`
- WHEN an authenticated dossiq API request arrives
...
- AND dossiq MUST NOT read a `maxStorageMb` field from a private tenant schema

---

### Requirement: TenantController API Surface MUST Be Preserved

All dossiq tenant API endpoints MUST remain available with identical HTTP paths, methods,
...
A one-time data migration script MUST transfer all existing dossiq `tenant` schema objects
to OR Organisation entities. The migration MUST be idempotent and MUST preserve object UUIDs
where possible.

#### Scenario: Migration creates OR Organisation for each dossiq tenant

- GIVEN dossiq has 3 existing tenant objects in its `tenant` OR schema
- WHEN the migration script `occ dossiq:migrate-tenants` runs
- THEN 3 OR Organisation objects MUST exist after the run
- AND each Organisation MUST have the same UUID as the corresponding dossiq tenant object
...
- GIVEN dossiq tenant object `T1` has `isActive: true` and tenant object `T2` has `isActive: false`
...
- GIVEN dossiq tenant object `T1` has `maxUsers: 50`
- WHEN the migration runs
...
### Requirement: Dossiq Tenant Schema MUST Be Deprecated After Migration

dossiq's `tenant` schema entry in `dossiq_register.json` MUST be marked deprecated after
...
- WHEN `dossiq_register.json` is updated
...
- GIVEN dossiq's `tenant` schema is marked deprecated after migration
...
dossiq MUST include integration tests that confirm TenantMiddleware correctly delegates to
...
- WHEN the test calls a dossiq API endpoint as that user
...
- GIVEN a test that inserts 2 dossiq `tenant` objects (one `isActive: true`, one `isActive: false`)
