...
- WHEN an admin enables the Dossiq app via `php occ app:enable dossiq`
- THEN the app MUST activate without errors
- AND it MUST register a navigation entry in the top bar pointing to `dossiq.dashboard.page`
- AND the `InitializeSettings` repair step MUST run to create/detect the Dossiq register
...
- WHEN a user navigates to `/apps/dossiq/`
...
- AND `APP_ID` MUST be the constant `'dossiq'`
...
- WHEN the user navigates to `/apps/dossiq/`
- THEN `DashboardController::page()` MUST return a `TemplateResponse` with the `main` template
- AND it MUST call `Util::addScript('dossiq', 'dossiq-main')` to load the SPA bundle
- AND it MUST call `Util::addStyle('dossiq', 'dossiq-main')` if a separate CSS bundle exists
...
- THEN the router MUST use history mode with base URL `generateUrl('/apps/dossiq')`
...
- THEN it MUST produce `js/dossiq-main.js` for the dashboard SPA
- AND it MUST produce `js/dossiq-settings.js` for the admin settings page
- AND it MUST produce `js/dossiq-casesOverviewWidget.js`, `js/dossiq-overdueCasesWidget.js`, and `js/dossiq-myTasksWidget.js` for Nextcloud Dashboard widgets
...
- THEN all strings MUST use `t('dossiq', 'key')` for simple strings
- AND parameterized strings MUST use `t('dossiq', 'Updated: {fields}', { fields: changes.join(', ') })`
- AND plural strings MUST use `n('dossiq', '{count} task', '{count} tasks', count)`
...
- THEN `<admin>OCA\Dossiq\Settings\AdminSettings</admin>` MUST be registered
- AND `<admin-section>OCA\Dossiq\Sections\SettingsSection</admin-section>` MUST be registered
- AND the section MUST appear under "Administration" in the settings sidebar with the Dossiq icon

#### Scenario 6.2: Admin settings page rendering
- GIVEN an admin user navigating to `/settings/admin/dossiq`
- WHEN the settings page loads
- THEN it MUST load the `dossiq-settings.js` bundle
- AND it MUST display the current register ID and schema IDs for all configured schemas
- AND it MUST include a "Reload configuration" button to re-import from `dossiq_register.json`

#### Scenario 6.3: In-app settings views
- GIVEN the Settings route at `/apps/dossiq/settings`
...
- GIVEN the CaseTypes route at `/apps/dossiq/case-types`
...
- GIVEN an admin user
- WHEN a POST request is made to `/apps/dossiq/api/settings` with JSON body containing config keys
...
- THEN each schema slug from `dossiq_register.json` MUST have a corresponding entry in this mapping
...
- THEN it MUST call `SettingsService::loadConfiguration()` to import `dossiq_register.json`
...
- GIVEN the file `lib/Settings/dossiq_register.json`
- WHEN the register is imported
- THEN the JSON MUST follow OpenAPI 3.0.0 format with `info.title: "Dossiq"`, `info.version`, and schema definitions
...
The app source code MUST be hosted at `ConductionNL/dossiq` on GitHub.

#### Scenario 9.1: Repository exists and is public
- GIVEN the ConductionNL GitHub organization
- WHEN checking for the dossiq repository
- THEN `https://github.com/ConductionNL/dossiq` MUST exist and be public
...
- THEN the main list MUST include items for: Dashboard (ViewDashboard icon), My Work (AccountCheck icon), Cases (FolderOpen icon), Tasks (ClipboardCheckOutline icon), Documentation (BookOpenVariantOutline icon, external link to https://conduction.nl)
...
- THEN it MUST call `window.open('https://conduction.nl', '_blank')` to open in a new tab
...
- AND clicking items in the widget MUST navigate to the Dossiq app
...
- THEN `DeepLinkRegistrationListener` MUST register deep links so OpenRegister can link directly to Dossiq case and task detail views
- AND the deep links MUST map object types (case, task) to Dossiq routes
...
- **App registration**: `appinfo/info.xml` -- id `dossiq`, name "Dossiq" (en/nl), namespace `Dossiq`, Nextcloud 28-33 compatibility, PHP 8.1+ requirement. Categories: organization, tools, workflow.
...
- **Translation support**: `t('dossiq', ...)` used in all Vue components. Global mixin in `main.js`.
...
- **GitHub repository**: https://github.com/ConductionNL/dossiq.
...
- **OpenAPI 3.0.0**: Register configuration format for `dossiq_register.json`.
...
### REQ-013: InitializeSettings repair step SHALL auto-import the dossiq register on app enable, gracefully no-op when OpenRegister is unavailable

`OCA\Dossiq\Repair\InitializeSettings` SHALL implement `IRepairStep` and SHALL run on every app enable / upgrade. The `run(IOutput $output)` method SHALL:
- Short-circuit with a warning when `SettingsService::isOpenRegisterAvailable()` returns false — never throw.
- Otherwise invoke `SettingsService::loadConfiguration(force: true)` to re-import `dossiq_register.json` and auto-configure every schema/register ID.
...
- NEVER propagate exceptions back to the repair runner — a Dossiq install must succeed even when the import partially fails (per the canonical Repair-step pattern documented in MEMORY.md).

#### Scenario: OpenRegister missing -> graceful warning
- **GIVEN** the `openregister` app is not installed
- **WHEN** `InitializeSettings::run()` executes during `occ app:enable dossiq`
...
- **GIVEN** OpenRegister is available and `dossiq_register.json` is valid
- **WHEN** the repair step runs
- **THEN** `loadConfiguration(force: true)` SHALL be invoked
- **AND** on `success: true` the step SHALL emit `Dossiq configuration imported successfully (version: <X>)`
...
- **THEN** the step SHALL catch the throwable, emit `$output->warning('Could not auto-configure Dossiq: ...')`, log an error, and return — never propagate.

#### Notes
- Per MEMORY.md ("Import OR register via Repair step, NOT Migration"): this is the only place in Dossiq's install flow that touches OpenRegister registers. The legacy migration-time import path silently skipped because peer-app autoloaders weren't loaded yet.

---

### REQ-014: SeedDataService SHALL idempotently seed bezwaar/beroep case types from a bundled JSON template

`OCA\Dossiq\Service\SeedDataService::seedBezwaarBeroepData()` SHALL:
