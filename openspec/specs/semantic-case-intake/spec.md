# semantic-case-intake Specification

**Status:** done
**Scope:** dossiq (consume-side of the `ns#Case` handoff)
**Depends on:** hydra change `semantic-object-handoff` (ADR-051) — `x-openregister-handoff` dialect; OpenRegister `SemanticTypeResolver` + Handoff engine (MERGED on origin/development); ADR-048 semantic references; ADR-031 notification dialect
**Backs:** README "Pipelinq Bridge" feature claim
**OpenSpec changes**: [semantic-case-intake](../../changes/archive/2026-07-06-semantic-case-intake/) _(archived 2026-07-06)_

## Purpose

Dossiq provides the semantic kind `https://openregister.app/ns#Case`: pipelinq requests handed
off to that kind are created as dossiq cases with faithful field mapping and navigable
...
Dossiq's `case` schema SHALL declare `implements: ["https://openregister.app/ns#Case"]` so
OpenRegister's `SemanticTypeResolver` resolves dossiq as a `ns#Case` provider. Discovery SHALL
degrade gracefully per ADR-048: when dossiq is not installed, the handoff resolves to no provider
and pipelinq keeps functioning standalone.

#### Scenario: Resolver discovers dossiq as Case provider

@e2e exclude Provider discovery runs inside OpenRegister's SemanticTypeResolver + complete-binding filter (backend, cross-app); dossiq's contribution is the implements + handoffContract declaration, proven by PHPUnit (SemanticCaseIntakeTest) against the REAL HandoffKindContracts ns#Case set. No dossiq browser surface.

- **GIVEN** dossiq is installed and its register configuration is imported
- **WHEN** OpenRegister resolves providers for `https://openregister.app/ns#Case`
- **THEN** dossiq's case schema MUST be returned as a provider

#### Scenario: Absent provider degrades gracefully

@e2e exclude ADR-048 graceful-degrade is OpenRegister/pipelinq behaviour (no dossiq provider present); nothing to drive in a dossiq UI. Covered by OR's handoff engine tests.
...
brief's `provenance` is that `source` field.) Dossiq SHALL NOT expose an app-local creation
endpoint for the handoff (creation flows through OpenRegister).

#### Scenario: Pipelinq request becomes a dossiq case

@e2e exclude End-to-end handoff execution spans pipelinq's produce-side + OpenRegister's HandoffService (target create, binding translation, provenance relations) — a cross-app backend flow with no dossiq-only browser path. The faithful, complete binding of the mapped fields is proven by PHPUnit (SemanticCaseIntakeTest) against the real contract.

- **GIVEN** a pipelinq request with title, summary, requester, channel, and priority
- **WHEN** it is handed off to `ns#Case` and dossiq is the resolved provider
- **THEN** a dossiq case MUST exist with the mapped title, description, intakeChannel, and priority
...
`x-openregister-notifications` dialect on the case schema (ADR-031); dossiq SHALL NOT dispatch
...
@e2e exclude The notification fires from OpenRegister's AnnotationNotificationDispatcher reading the schema's x-openregister-notifications rule (created + notIn filter on handoffSource); PHPUnit proves the declaration is present and that lib/ contains no imperative handoff-intake dispatch. No dossiq UI to drive.
...
- **AND** no dossiq PHP code MUST imperatively dispatch the intake notification
