...
## Purpose
TBD - created by archiving change migrate-sla-dashboard-to-analytics-leaf. Update Purpose after archive.
## Requirements
### Requirement: Doorlooptijd Charts Render Through The OR Analytics Leaf

Dossiq SHALL render the doorlooptijd (SLA) dashboard charts through OpenRegister's `analytics`
integration leaf (ADR-019). Dossiq SHALL NOT embed a chart library (apexcharts) directly in the
dashboard view after this migration.

@e2e exclude The chart render is driven by OpenRegister's page-level analytics-series surface (cross-app, ADR-019) — series are registered via `POST /api/integrations/analytics/series` and drawn by `@conduction/nextcloud-vue`'s declarative `CnChartWidget` (which owns the chart engine). The dossiq-side change is the removal of the bespoke `vue-apexcharts` import/option-shaping (a static no-direct-chart-lib check, verified by code review + the absence of the dep in `package.json`) plus the series-registration wiring; none is a dossiq-only browser UI surface drivable without the OR analytics-series surface installed. Mirrors the case-map-via-maps-leaf / inspection-forms-via-forms-leaf precedent.
...
Dossiq SHALL retain the SLA compliance calculation (`doorlooptijdHelpers.js`:
