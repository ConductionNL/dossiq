...
to the source record: the person's or the organisation's page in this app for a `brpPerson` or
`kvkCompany` row, and the contact otherwise. Cases without an initiator SHALL show no empty
initiator block.
...
- **AND** the KvK number MUST link to the organisation page of the seeded `kvkCompany` record
