...
### Requirement 2: Object store MUST register all Dossiq entity types dynamically
The store MUST support registering all Dossiq object types at runtime from settings configuration.

#### Scenario 2.1: Register core entity types
- GIVEN the settings config returns IDs for all Dossiq schemas
...
- THEN it MUST GET `/apps/dossiq/api/settings` with headers `requesttoken` and `OCS-APIREQUEST`
...
- THEN it MUST POST to `/apps/dossiq/api/settings` with the JSON body
...
**All requirements are implemented via the shared library and Dossiq-specific initialization code.**
...
This spec is highly detailed with 10 requirements covering store creation, type registration, collection/object fetching, CRUD, loading/error states, settings initialization, authentication, composable usage, and page component integration. It accurately describes both the shared library pattern and Dossiq-specific usage including the 13+ registered object types, CaseList/CaseDetail integration patterns, and sidebar state management.
