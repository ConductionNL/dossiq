---
status: done
status-note: "Synced 2026-06-14 from archived consolidation change add-dossiq-procurement-suite (kind:config). SPEC-COMPLETE; code chain pending (ADR-032). Reuses dossiq's existing decision register for awards (no Award register) per T16/REQ-EVA-001."
---

# dossiq-procurement-evaluation-award Specification

## Purpose
Define bid evaluation + award for dossiq: scoring as an `Evaluation`
register (one per bid per evaluator), awards as additive fields on
dossiq's existing `decision` register, data-driven scoring formulas,
...
## Requirements
### Requirement: REQ-EVA-001 — Scoring SHALL be an `Evaluation` register attached to a Bid; the existing dossiq `decision` register SHALL carry the award

Scoring SHALL be a new `Evaluation` register and the award SHALL reuse dossiq's existing `decision` register; dossiq MUST NOT add an `Award` register.

To avoid duplicating dossiq's existing `decision` register, this spec
...
2. **Award outcome** — reuses dossiq's *existing* `decision` register
...
Dossiq MUST NOT define an `Award` register that mirrors `decision`.
...
- **GIVEN** the dossiq register file
...
### Requirement: REQ-EVA-002 — Award decisions SHALL be dossiq `decision` records with seeded `decisionType`s

Dossiq MUST seed three `decisionType` records for procurement:
...
Scoring formulas SHALL be declared as register data; dossiq MUST NOT hardcode formula behaviour in a service.
...
Dossiq MUST NOT author a `ScoringFormulaService` switch statement
...
- **THEN** new tenders MUST be able to reference the new formula via
  `awardCriteria[i].scoringMethod`; no dossiq PHP changes.

### Requirement: REQ-EVA-004 — Calibration sessions SHALL be a child case under the tender, reusing existing dossiq case machinery

Calibration sessions SHALL be modelled as child cases reusing dossiq case machinery; dossiq MUST NOT author a parallel session service.

A calibration session (where evaluators reconcile divergent scores)
MUST be a dossiq child case under the tender (`caseType:
...
Dossiq MUST NOT author a `CalibrationSessionService` parallel to
`case`.
...
The standstill (Alcatel-termijn) SHALL be enforced by the declarative `Tender` lifecycle; dossiq MUST NOT author a standalone finalisation guard service.
...
Dossiq MUST NOT author an `AwardFinalisationGuard` PHP class beyond
...
### Requirement: REQ-EVA-006 — Award documents SHALL be generated by docudesk, not by dossiq

Award documents SHALL be generated by docudesk; dossiq MUST NOT author PDF or letter rendering services.
...
seeded as part of the EVA implementation chain). Dossiq MUST NOT
author a `GunningPdfService`, `RejectionLetterService`, or
`MotiveringRenderer`.

The decision register MUST carry a `documentRef` field (already
present in dossiq's existing `decision` schema as
`decisionDocument`) populated with the docudesk-generated URI.

#### Scenario: Reviewer scans for forbidden PDF generation in dossiq
...
### Requirement: REQ-EVA-007 — Evaluation + award pages SHALL be reachable through the dossiq manifest navigation
