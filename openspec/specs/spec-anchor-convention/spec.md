# spec-anchor-convention Specification

## Purpose

A `@spec` or `@e2e` tag in code says which requirement the code answers to. This
capability says what such a tag is allowed to name, so that it stays true without
being edited.

A spec has three homes in its life: an open change's delta, that change's archived
delta, and the canonical `openspec/specs/<capability>/spec.md`. A tag that names a
path moves out of date every time the spec moves between them. A tag that names the
capability never does. So the tag names the capability, is written once, and is
resolved by looking in all three homes.

The rule also covers the case where the tag is early: a capability whose canonical
spec has not been written yet is still named in its canonical spelling, and the
archive is what brings the content to the path the tag already pointed at.

Where nothing describes what a test proves, the test says so with a reasoned
`@e2e exclude` rather than citing the nearest plausible capability. A citation that
resolves while naming the wrong requirement reads as verified coverage and passes
every mechanical check, which makes it worse than one that does not resolve.
