---
status: done
note: Implemented and archived 2026-06-13 (change tenant-zaaksysteem-saas-08). TenantConfigurationService (branding/locale/feature-flags), logo-upload validation, CSS theming tokens, and custom-CSS XSS sanitisation shipped. Let's Encrypt cert install + subdomain→tenant resolution are deferred ([~]) as infrastructure (reverse-proxy/domain provisioning), not app code.
---

# tenant-configuration Specification

## Purpose
Per-tenant configuration and branding for the multi-tenant SaaS: persists branding/locale/timezone/feature-flags, validates logo uploads (MIME + size), emits NL Design System CSS theming tokens, and sanitises enterprise custom CSS against an XSS-safe property whitelist.
## Requirements
