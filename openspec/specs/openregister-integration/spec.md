...
All frontend `fetch()` calls to internal Procest API endpoints MUST construct URLs using `generateUrl()` from `@nextcloud/router`. Hardcoded path strings are prohibited.
...
**Rationale**: `generateUrl()` produces the correct URL regardless of whether `mod_rewrite` / `index.php` stripping is enabled on the Nextcloud installation. Hardcoded paths produce 404 errors on standard installations.

**Scope**: Applies to ALL Pinia stores calling Procest's own backend controllers (`SettingsController`, `ZgwMappingController`). Does NOT apply to calls to the OpenRegister API (`/index.php/apps/openregister/api/...`), which are handled separately.

#### Scenario: Settings fetch uses generateUrl

- GIVEN a Procest app page load
- WHEN `fetchSettings()` is called
- THEN the network request MUST go to the correct URL (no 404)

#### Scenario: ZGW mappings fetch uses generateUrl

- GIVEN the Procest admin settings page
- WHEN ZGW mappings are fetched
- THEN the network request MUST go to the correct URL (no 404)

#### Scenario: App initializes without cascade errors

- GIVEN the fix is applied
- WHEN the app initializes
- THEN `initializeStores()` MUST complete without "Object type X is not registered" errors
