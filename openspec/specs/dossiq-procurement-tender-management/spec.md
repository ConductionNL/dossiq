...
# dossiq-procurement-tender-management Specification

## Purpose
Define tender (aanbesteding) management for dossiq: tenders as dossiq
...
## Requirements
### Requirement: REQ-TND-001 — Tenders SHALL be modelled as dossiq cases (`schema:Project`), not as a parallel domain object

A tender (aanbesteding, aanbestedingsdossier) MUST be modelled as a
dossiq `Case` of a seeded `caseType: tender` (Schema.org
`schema:Project`). This reuses dossiq's existing case-management,
...
- **WHEN** that user opens the dossiq my-work dashboard
...
Per ADR-031, dossiq MUST NOT author `TenderService::transition*` or
...
`dossiq-procurement-system-integration`). Dossiq MUST NOT author
...
be referenced by URI — not stored in dossiq tables.
...
- **THEN** the document MUST be authored in docudesk (using docudesk's
  template engine — not in dossiq's `lib/`), with each question's
  `noi` field pointing back to the resulting URI.

### Requirement: REQ-TND-005 — Multi-lot tenders SHALL reuse dossiq's existing `deelzaak-support`

When a tender has lots, each lot MUST be modelled as a child case
(deelzaak) under the parent tender case, using dossiq's existing
...
- `bezwaarOpen` — boolean, true while any linked bezwaar case (dossiq
  `bezwaar-lifecycle`) is in a non-terminal state.

Dossiq MUST NOT author `TenderTermijnService` or
...
dossiq MUST NOT author a `BidLifecycleService`.
...
Dossiq MUST NOT author `TenderNotificationService`.
...
### Requirement: REQ-TND-009 — Tender registers SHALL be reachable through the dossiq manifest navigation
...
- **WHEN** an inkoper opens `/index.php/apps/dossiq/tenders`
- **THEN** the page MUST render via `CnIndexPage` showing tender
  cases — no other case types appear, no dossiq-side filter
