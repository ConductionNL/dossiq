---
status: done
status-note: "Synced 2026-06-14 from archived consolidation change add-dossiq-procurement-suite (kind:config). SPEC-COMPLETE; code chain pending (ADR-032). Concrete openconnector source rows for the declared slots land in a separate add-openconnector-eu-procurement-sources change."
---

# dossiq-procurement-system-integration Specification

## Purpose
Define dossiq's external-procurement integration posture: dossiq
...
failures surfaced as dossiq tasks — zero transport code in dossiq lib/.
## Requirements
### Requirement: REQ-PSI-001 — Dossiq SHALL declare logical connector slots; openconnector SHALL own transport

Per ADR-019 + ADR-022, dossiq MUST NOT author transport code
(`*Client`, `*HttpService`, `curl_init`, `GuzzleHttp\Client`) for any
external procurement system. Dossiq declares **logical connector
slots** — symbolic names that downstream openconnector source rows
fulfil. The slot is a property of the relevant dossiq register
...
- **WHEN** dossiq dispatches a publish event for a Tender
- **THEN** the dispatch MUST resolve via OR's `ScheduledWorkflow` →
  openconnector source lookup, with no per-app HTTP client.

### Requirement: REQ-PSI-002 — Inbound integration events SHALL flow through OR's integration registry, not a dossiq webhook controller

Inbound integration events SHALL flow through OR's integration registry; dossiq MUST NOT define a webhook controller for external systems.

External systems that push to dossiq (Mercell bid received, TenderNed
...
endpoint, OR's CloudEvent dispatcher, then dossiq's domain handlers.

Dossiq MUST NOT define `lib/Controller/*WebhookController.php` for
...
- **THEN** openconnector MUST dispatch a `procurement.bid.received`
  CloudEvent on the OR bus; dossiq's declarative `Bid` lifecycle
  MUST consume it via `x-openregister-lifecycle.requires` — no
  dossiq webhook controller is invoked.

### Requirement: REQ-PSI-003 — Connector slot mapping SHALL be declared as schema metadata, not as code

Connector slot mapping SHALL be declared as schema metadata; dossiq MUST NOT author a connector-registry resolution service.

Each slot's mapping (which dossiq event triggers which slot, which
...
Dossiq MUST NOT author a `ConnectorRegistryService` that hardcodes
...
- **THEN** no dossiq PHP code MUST change.
...
Dossiq MUST NOT author a `KvkLookupService` HTTP wrapper.
...
### Requirement: REQ-PSI-005 — Outbound integration failures SHALL surface in dossiq as task signals, not as silent retries

Outbound integration failures SHALL surface as dossiq `task` records on the relevant case; dossiq MUST NOT author a parallel failure-log register.
...
dossiq `Task` (reusing the existing dossiq `task` register) on the
...
Dossiq MUST NOT author a parallel `IntegrationFailureLog` register
...
`@conduction/nextcloud-vue`'s `CnIntegrationsPage`). Dossiq MUST NOT
...
- **THEN** the page MUST render `CnIntegrationsPage` from
  ...
  dossiq's procurement suite (no dossiq-side admin component).
