---
status: done
status-note: "Synced 2026-06-14 from archived consolidation change add-dossiq-procurement-suite (kind:config). SPEC-COMPLETE; cross-app contract. The posting/spend side depends on [future] financeq (repo does not yet exist); launchpad owns the analytics surface (ADR-024 §10) and consumes dossiq via runtime OR GraphQL — no install-time dependency."
---

# dossiq-procurement-spend-analytics-integration Specification

## Purpose
Define the cross-app spend-analytics contract: dossiq emits procurement
CloudEvents and exposes its registers via OR GraphQL; launchpad consumes them
at runtime to render spend analytics. Commitment side from dossiq,
posting side from [future] financeq. RBAC stays OR-canonical; dossiq ships
no analytics manifest entry (launchpad owns the surface).
## Requirements
### Requirement: REQ-PSA-001 — Dossiq SHALL emit procurement-domain CloudEvents; launchpad SHALL consume them via runtime GraphQL

Dossiq SHALL emit procurement-domain CloudEvents via OR's event pipeline; launchpad SHALL consume them at runtime and MUST NOT declare an install-time dependency.

This spec is a **cross-app contract** spec. Dossiq emits domain
...
MUST NOT declare dossiq, openregister, or financeq as install-time
dependencies — the consumption is runtime-only via OR's GraphQL
endpoint.

Dossiq MUST NOT author analytics widgets, dashboards, or KPI
...
Dossiq MUST emit CloudEvents (per OR's existing
...
the changed object's id + the field delta. Dossiq MUST NOT author a
...
- **THEN** no such dossiq-specific event-machinery classes SHALL
  exist.

### Requirement: REQ-PSA-002 — Dossiq SHALL expose a procurement GraphQL schema slice via OR's GraphQL abstraction

LaunchPad's spend-analytics widgets MUST query dossiq data via OR's
GraphQL endpoint (per ADR-022 row "Schema declarative extensions" +
GraphQL exposure). Dossiq MUST NOT author a custom REST surface for
launchpad — the existing OR GraphQL is the only consumer-side contract.

The dossiq registers (`Tender`, `Contract`, `Supplier`, `Bid`,
`Evaluation`, `decision` with procurement decisionTypes,
`PublicationNotice`) MUST be GraphQL-queryable with declarative
filters declared in the schema metadata. No bespoke dossiq GraphQL
resolver code.

#### Scenario: LaunchPad queries dossiq contracts via GraphQL
...
- **THEN** the OR GraphQL endpoint MUST serve the response, gated by
  OR RBAC; dossiq's code path MUST NOT contain a `GraphQLResolver`
...
MUST be the same OR roles dossiq uses internally
...
- **WHEN** they load the spend-analytics widget querying dossiq
  contracts
- **THEN** the OR GraphQL response MUST be empty (RBAC-filtered);
  launchpad MUST surface "no data" without a stack trace.

### Requirement: REQ-PSA-004 — Aggregate spend calculations SHALL forward to `[future]` financeq, not be computed in dossiq

Aggregate posted-spend calculations SHALL come from financeq; dossiq MUST NOT compute the posting side and MUST mark the gap as `[future]`.
...
NOT from dossiq. Dossiq provides the *commitment* side (contract
valueAmount, tender estimatedValue, award value); financeq provides
the *posting* side. Until financeq exists:

- dossiq MUST mark these analytics gaps as `[future]` in the
...
### Requirement: REQ-PSA-005 — Dossiq MUST NOT ship a spend-analytics manifest entry; launchpad owns the surface

`dossiq/src/manifest.json` MUST NOT declare a `Procurement >
...
Dossiq MAY declare a deep-link convention (OR's `deep link registry`)
so launchpad widgets can link back to individual dossiq objects
...
#### Scenario: Reviewer confirms no dossiq-side analytics page
...
- **THEN** no such entry SHALL exist in dossiq's manifest; cross-app
  analytics is launchpad's surface.

#### Scenario: A launchpad widget deep-links to a dossiq contract

- **GIVEN** a launchpad spend widget showing the top-10 contracts by
  commitment value
- **WHEN** a user clicks one
- **THEN** the link MUST route to
  `/index.php/apps/dossiq/contracts/<uuid>` via the OR deep-link
