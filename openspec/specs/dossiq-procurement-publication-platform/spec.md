---
status: done
status-note: "Synced 2026-06-14 from archived consolidation change add-dossiq-procurement-suite (kind:config). SPEC-COMPLETE; code chain pending (ADR-032). Depends on the openconnector TED/TenderNed source rows from add-openconnector-eu-procurement-sources."
---

# dossiq-procurement-publication-platform Specification

## Purpose
Define TED/OJEU + national notice publication for dossiq: notices as a
...
TED eForms standard-form codes SHALL be seeded as a `PublicationTemplate` register; dossiq MUST NOT hardcode them as enums.
...
Per ADR-031, dossiq MUST NOT author `PublicationNoticeService::
transition*` methods.
...
Material changes SHALL be surfaced by a declarative calculation; dossiq MUST NOT author a material-change detector service.
...
`isMaterialChange` on `Tender` MUST surface `true`, and dossiq MUST
recommend publishing a `rectificatie` (or `wijziging-opdracht` after
award) notice.

Dossiq MUST NOT author a `MaterialChangeDetectorService` — per
...
openconnector source resolved from `targetPlatformSlot`. Dossiq MUST
...
- **THEN** an OR `ScheduledWorkflow` MUST be dispatched targeting the
  `tenderned-tenders` source with the payload; dossiq's code path
  MUST not invoke any HTTP client directly.

### Requirement: REQ-PPP-006 — Publication pages SHALL be reachable through the dossiq manifest navigation
...
- **WHEN** an inkoper opens
  `/index.php/apps/dossiq/publications?targetPlatform=ted-ojeu`
- **THEN** the page MUST render via `CnIndexPage` showing only TED
  notices — no dossiq-side filter controller invoked.
