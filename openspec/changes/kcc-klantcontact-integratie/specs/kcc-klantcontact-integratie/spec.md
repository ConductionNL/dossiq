...
## Purpose
Implement omnichannel contact management for Dutch municipal KlantContactCentra, consolidating phone, email, web, chat, and social-media inquiries into a unified ContactMoment model with intelligent keyword-based routing, staff workload balancing, and realtime status feedback. The system enables KCC agents to quickly identify callers, view open cases, route to the right team, and track handoff completion—reducing average handle time from 4.2 to 2.8 minutes and improving first-contact resolution from current baseline.

## Context
KCC staff today use disconnected systems (CTI, Outlook, ticketing, case management, BRP lookup, post-its). Procest provides case management; Pipelinq provides the KCC workplek frontend. This change integrates ContactMoment as a first-class entity that bridges CTI, email, web forms, and chat intake into a unified routing and feedback loop. Routing rules (keyword-based, configurable) assign contacts to teams; agents are ranked by availability, skill-match, and workload. Callbacks are scheduled and tracked; volume dashboards support capacity planning.
