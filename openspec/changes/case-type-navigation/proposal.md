...
- **Frontend adopts the backend delta** — `src/main.js` routes the built manifest through `useAppManifest('procest', built, { mergeStrategy: 'delta' })` and passes the reactive resolved manifest to `App.vue`, so the nav updates in place when the delta lands.
