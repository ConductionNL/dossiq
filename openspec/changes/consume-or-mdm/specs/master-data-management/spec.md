...
# Spec: master-data-management (procest as ADR-045 consumer)

**Status:** proposed
**Scope:** procest (annotations only — engine and steward surface are OpenRegister's per ADR-045)
**Depends on:** OpenRegister MDM engine (`x-openregister-quality` / `x-openregister-dedup` support, on-save score materialisation)
