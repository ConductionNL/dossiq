...
- [x] 1.1 `lib/Service/Email/NextcloudMailGateway.php`: the one class that
...
- [x] 1.2 `lib/BackgroundJob/InboundEmailJob.php`: stop calling
  `imap_open()`, listen for `OCA\Mail\Events\NewMessagesSynchronized`, and
  read the account an administrator picked (D-2).
- [x] 1.3 `src/views/settings/EmailSettings.vue`: replace the host,
  username and password fields with an account picker.
- [x] 1.4 A migration that deletes `email_imap_password`,
  `email_imap_username` and `email_imap_host` (D-2).
  - `tests/Unit/Repair/RetireImapCredentialsTest.php`
- [x] 2.1 `lib/Service/Email/Filters/`: the named, ordered pipeline with
...
- [x] 2.2 The first filters: auto-reply, bounce notification, own
  notification loop, and out of office (D-3).
- [x] 3.1 `bounce` and `move` as named acts, with the forwarding address
  and the reason recorded, no case created and no rejection sent (D-4).
  - `tests/Unit/Service/Email/BounceActionTest.php`
- [x] 3.2 `lib/Service/Email/UnmatchedMailIntake.php`: an unmatched
  message lands in the intake inbox with its verdict, never dropped (D-5).
- [x] 4.1 `lib/Service/Email/AuthenticationVerdict.php`: SPF and DMARC
