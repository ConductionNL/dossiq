...
- [~] TASK-GIS-02: Create `lib/Service/PdokService.php` with PDOK Locatieserver integration: `searchAddress()` with autocomplete caching, `searchParcel()` by number or bbox, error handling for PDOK outages, and local caching layer using Nextcloud cache with 1-hour TTL. Add integration tests with mock PDOK responses. **Adapter layer SHIPPED upstream 2026-06-11** as dormant openconnector source adapters at `openconnector/lib/Sources/Pdok/PdokWmsSourceAdapter.php`, `openconnector/lib/Sources/Pdok/PdokWfsSourceAdapter.php`, `openconnector/lib/Sources/Pdok/PdokGeocodingClient.php` (category=`geo`, flipped live via `pdok.feature_flag`). Procest-side PdokService should consume these via the `migrate-pdok-to-openconnector` HTTP shim rather than re-implement Locatieserver inside procest (ADR-022).
...
Of the 30 spec tasks, 3 are already ticked above. The remaining 27 were
mechanically flipped to `[~]`. The reality is **more is shipped than the spec
admits** — the deferred ticks are a documentation lag, not a code gap:

- `lib/Service/MapLayerService.php` (CRUD + payload validation)
- `lib/Service/GisProxyService.php` (PDOK/WMS/WFS proxy)
- `lib/Service/WfsExportService.php` (OGC WFS XML)
- `lib/Controller/MapLayerController.php` + `GisProxyController.php`
- Routes `gisProxy#proxy/capabilities` + WMS/WFS layer proxy at appinfo/routes.php:206-209
- Vue: `src/components/map/MapComponent.vue`, `CaseMap.vue`, `LocationPicker.vue`,
  `MapLayerSwitcher.vue`, `MapLegend.vue`; `src/views/dashboard/CaseMapWidget.vue`;
  `src/views/cases/components/LocationTab.vue`; `src/views/settings/MapLayerSettings.vue`
- Backed by: openconnector PDOK connector (per `migrate-pdok-to-openconnector`)

Real gaps that justify the `[~]`:
- Public `/wfs/cases` OGC WFS endpoint is partial; only `WfsExportService` ships the
  GetCapabilities/GetFeature/DescribeFeatureType paths; the public mount + CORS config
  ride on the next OpenConnector PDOK migration.
- Frontend `CasesOnMapView` (clustered marker dashboard) is shipped as
  `CaseMapWidget.vue` on the dashboard — the spec calls for a full-screen view
  which is a UX refinement, not a missing capability.
- Performance testing + WFS-endpoint CORS verification require a live deployment.

A focused `gis-integration-v2-cleanup` ghost change is queued to re-tick the
existing implementation paths after the documentation lag is resolved.
