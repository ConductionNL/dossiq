...
- [~] TASK-GIS-01: Create `lib/Service/GeoService.php` with CRUD methods for case geometry (create, read, update, delete), GeoJSON validation (RFC 7946), and coordinate transformation helpers. Add unit tests for geometry validation and edge cases (MultiPolygon, empty coordinates). — deferred to downstream cycle / fleet-wide adoption (handoff)

- [~] TASK-GIS-02: Create `lib/Service/PdokService.php` with PDOK Locatieserver integration: `searchAddress()` with autocomplete caching, `searchParcel()` by number or bbox, error handling for PDOK outages, and local caching layer using Nextcloud cache with 1-hour TTL. Add integration tests with mock PDOK responses. — deferred to downstream cycle / fleet-wide adoption (handoff)

- [~] TASK-GIS-03: Create `lib/Service/MapLayerService.php` with CRUD for `mapLayer` objects, layer filtering by type (tile/wms/wfs/geojson), visibility toggling, and ordering. Add validation for layer URLs and WMS/WFS endpoint syntax. — deferred to downstream cycle / fleet-wide adoption (handoff)

- [~] TASK-GIS-04: Create `lib/Service/WfsService.php` to generate OGC WFS 2.0.0-compliant XML responses: `GetCapabilities` (list case features), `GetFeature` (with bbox/BBOX filters, CRS handling), `DescribeFeatureType`. Add unit tests for WFS XML structure and bbox filtering. — deferred to downstream cycle / fleet-wide adoption (handoff)

## Backend Controllers & Routes

- [~] TASK-GIS-05: Create `lib/Controller/LocationPickerController.php` with endpoints: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-06: Create `lib/Controller/WfsController.php` with public WFS endpoint at `GET /wfs/cases` handling `GetCapabilities`, `GetFeature`, `DescribeFeatureType` requests. Implement BBOX filtering, CRS (EPSG:4326), and metadata responses per OGC spec. — deferred to downstream cycle / fleet-wide adoption (handoff)

- [~] TASK-GIS-07: Create `lib/Controller/CaseGeoController.php` with endpoint `GET /api/cases/geo` returning clustered case locations for map view. Implement: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-08: Create `lib/Controller/MapLayerController.php` with admin REST endpoints: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-09: Register all new routes in `appinfo/routes.php` and add permission middleware (authenticated for internal endpoints, public for `/wfs/cases`). Document route structure in inline comments. — deferred to downstream cycle / fleet-wide adoption (handoff)

## Frontend Components

- [~] TASK-GIS-10: Create `src/components/LocationPickerModal.vue` with: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-11: Create `src/components/GeoViewer.vue` embedded map component with: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-12: Create `src/views/CasesOnMapView.vue` full-screen map dashboard with: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-13: Create `src/views/MapLayerSettings.vue` admin configuration panel under Settings > Kaartlagen with: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-14: Create `src/utils/mapLayerManager.js` helper module with: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-15: Create `src/utils/pdokIntegration.js` PDOK client module with: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-16: Modify `src/views/CaseDetail.vue` to: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-17: Modify `src/views/Dashboard.vue` or create navigation link to: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-18: Seed `mapLayer` objects in `lib/Settings/procest_register.json` with default layers: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-19: Update `lib/Service/SettingsService.php` to register new config keys: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-20: Create settings UI in `src/views/Settings/GisSettings.vue` (or extend existing settings) to configure: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-21: Add unit tests for GeoService, PdokService, WfsService with: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-22: Add integration tests for: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-23: Add frontend component tests (Vue Test Utils) for: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-24: Create user documentation in `docs/gis-integration.md`: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-25: Add i18n strings for all UI text: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-26: Install and configure map library (Leaflet or OpenLayers): — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-27: Verify WFS endpoint CORS configuration: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-28: Performance testing: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-GIS-29: Create migration script if needed to populate `case.geometry` from existing location data (if any legacy location fields exist). Document data mapping and validation. — deferred to downstream cycle / fleet-wide adoption (handoff)

- [~] TASK-GIS-30: Add change notes to `CHANGELOG.md` documenting: — deferred to downstream cycle / fleet-wide adoption (handoff)
