...
- [~] DEFERRED (needs live instance): Browser smoke — open CaseTypeDetail for "Omgevingsvergunning" → verify tabs render — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] DEFERRED (needs live instance): Results tab → add "Vergunning verleend" (retain, P20Y) → verify row appears — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] DEFERRED (needs live instance): Roles tab → add "Aanvrager" → verify row appears; delete → verify removed — deferred to downstream cycle / fleet-wide adoption (handoff)
