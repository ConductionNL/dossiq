...
- [x] 1.1 Lift `HearingService`'s Talk room creation to a case-level
...
- [x] 1.2 Keep the hoorzitting behaviour unchanged: scheduling, attendance
  and minutes (D-1).
  - `tests/unit/Service/Bezwaar/HearingServiceTest.php`
- [x] 1.3 No Talk, no affordance, with the case saying why (D-1).
- [x] 2.1 Record the conversation on the case: the moment, the
  participants and the duration (D-2).
  - `tests/vitest/caseConversationRecord.spec.js`
- [x] 2.2 File the recording, transcript or minutes as a case document
  through `documents-live-on-the-case`, under the case type's visibility
  declaration (D-2).
- [x] 3.1 Attach a voice note or a screen capture to a case or a task,
  landing as a case document; absent where the platform has no recorder
  (D-3).
  - `tests/unit/Service/CaseCaptureTest.php`
- [x] 4.1 `case.isMajor` as a permissioned act; `caseType`: the responders
  it names (D-4).
  - `tests/unit/Service/MajorCaseDeclarationTest.php`
- [x] 4.2 Declaring it opens exactly one channel and notifies the
  responders over the notification dialect, recording who and when (D-4).
- [x] 4.3 Refuse the declaration when the responders cannot be resolved,
  per ADR-102 (D-4).
- [x] 4.4 Close the channel with the case and file what was said on it
  (D-5).
- [x] 5.1 `tests/unit/Architecture/DeclaredToolAssumesNoTypingTest.php`:
  no declared tool names a keyboard affordance or a typed format (D-6).
- [x] 5.2 Confirm dossiq ships no speech recognition, and record
  C-communication-67 as hermiq's under D13.
- [x] 5.3 Dutch and English strings.
- [x] 5.4 `tests/e2e/live-conversation-on-the-case.spec.ts`: a
