# Design — spec-anchor-repair (procest)

## Category breakdown (base `origin/development`)

2,873 broken `@spec` anchors, classified by recoverability:

| Category | Count | Disposition |
|---|---|---|
| **(c) archived change → canonical spec recoverable** | **573** | **auto-repointed** |
| &nbsp;&nbsp;↳ exact requirement-heading match | 79 | `specs/<cap>/spec.md#requirement-<slug>` |
| &nbsp;&nbsp;↳ capability proven, heading not matched | 494 | `specs/<cap>/spec.md` (file-level) |
| **(d)**  no-fragment tasks.md ref, slug not a canonical capability | 1,081 | flag |
| **(d)**  non-tasks.md ref (decimal task / design.md / proposal.md / re-headed specs anchor) | 1,060 | flag |
| **(d)**  archived change dir not located | 110 | flag |
| **(d)**  change uses non-annotate tasks.md (no `task-N: cap#REQ` line) — needs spec-delta read | 49 | flag |
| **Total flagged for human triage** | **2,300** | `residual-dangling.md` |

Auto-repointed **20%**; the residual 2,300 stay broken *on purpose*
...
4. **Post-condition gate** — every proposed target is re-checked with gate-46
   ...
   and the anchor stays dangling. **Observed on procest: 0 rejects.**
...
573 anchors is infeasible and more error-prone, and the safety property
...
The residual 2,300 anchors are **not** force-removed or re-pointed on a
