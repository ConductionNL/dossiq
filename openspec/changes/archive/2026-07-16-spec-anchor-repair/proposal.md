...
on a mass scale — **2,873 in procest** on base `origin/development`, out of
3,605 total `@spec` tags (80%). A broken anchor is code *claiming* to
...
- **573 anchors repointed** across 216 files
  (79 to an exact requirement heading, 494 downgraded to
  capability-level file granularity).
- **2,300 anchors left untouched** and filed for human review
  (`residual-dangling.md` + umbrella issue) — never guessed.
- **gate-46 (`spec-anchor-existence`): 2,873 → 2,300 broken.**
- The diff is **`@spec`-comment-lines only**: 1,146 changed lines, **0**
...
- Gate-46 noise drops by 20%, so a *newly* broken anchor in a PR is now
  visible instead of drowning in 2,873 pre-existing failures.
