...
This change adds LLM-assisted redaction-span proposal to Woo document
disclosure review, as a human-reviewed ASSIST layered on top of the
existing deterministic PII-regex floor and the existing
`WOORedactionService` hand-off — never a replacement, never an
auto-redaction/auto-publish path.
