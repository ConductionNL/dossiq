...
**Scope:** procest
**Tier:** V1
**Depends on:** ADR-037 (modular config fragments + canonical `src/menu-layout.json` layout pass,
`applyMenuRelocations` / `applyMenuRemovals` in `src/main.js`), ADR-012 (deduplication).

## Purpose

De-duplicate procest's primary navigation (remove the two "Cases" / two "Analytics" / two
substitution entries) and collapse the flat operational work surfaces into top-level groups
(**Work**, **Cases**, **Analytics**) using the existing canonical-layout engine — without deleting
any page or changing any route. Out of scope and owned by sibling changes: the Objections/Appeals
cluster (`procest-objections-appeals-group`), the config→Settings relocations
(`procest-config-to-settings`), and decision delegation
(`procest-delegate-remaining-decisions-to-decidesk`).
