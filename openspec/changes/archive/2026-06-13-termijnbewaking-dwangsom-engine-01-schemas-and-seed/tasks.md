...
- [x] Run the integration test against a test OpenRegister instance — live-verified 2026-06-11 against the dev container at localhost:8080. All six termijnbewaking schemas materialise in OR: `termijnDefinitie` (id 931), `termijnInstance` (id 932), `termijnGebeurtenis` (id 933), `dwangsomBerekening` (id 935), `dwangsomUitbetaling` (id 936), `bewaarTermijnRegel` (id 942). All queryable via `/index.php/apps/openregister/api/objects/17/<schemaId>`. The `SeedTermijnDefinities` repair step reports "Termijnbewaking seed complete: 0 definities (0 overgeslagen)" on second invocation — proving idempotency.
