...
## Context
KCC-medewerkers today juggle separate CTI/telephony systems disconnected from the zaaksysteem. This capability adds a contact layer on top of Procest case management. Procest exposes the read/write API (contactmoment capture, case-voorblad resolution, quick-actions, belplan routing, sentiment scoring); the KCC-werkplek agent UI (screen-pop, dialogs, sentiment banner, specialist picker, klantreis timeline) is rendered by **pipelinq**. DigiD authentication is delivered by **OpenConnector**; the SIP warm transfer and live agent-availability feed by **pipelinq** telephony. This spec is `status: partial`: the Procest-side core is built; those cross-app legs are deferred.
