...
Add a GIS / geo capability for cases: a backend geometry service, an OGC WFS
2.0.0 endpoint exposing case locations to external GIS clients, a clustered
cases-on-map data endpoint, and a frontend map layer (a read-only single-case
geo viewer plus a full-screen cases-on-map dashboard). PDOK (Dutch geo base
layers / WFS) is reached via OpenConnector and degrades gracefully when
OpenConnector or PDOK is unavailable — the map never hard-fails.

**Standards**: GeoJSON (RFC 7946), OGC WFS 2.0.0, GML 3.2, EPSG:4326, PDOK tile/WFS services
**Feature tier**: V1

## Context

Per ADR-022, this spec consumes existing OpenRegister + OpenConnector
abstractions and does not re-implement them. Case location data is stored on
the `location` schema via the OpenRegister object store (geometry persisted
JSON-encoded — stringify on write, parse on read), and all external PDOK access
routes through the OpenConnector PDOK shim (`/apps/openconnector/api/pdok/*`).
