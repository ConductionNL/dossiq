...
ZGW DRC-compliant case dossier management for Procest. Every zaak gets a structured document
dossier backed by Nextcloud Files, where each linked file is represented as an `informatieobject`
with full ZGW metadata and joined to the case via `zaakinformatieobject`. The dossier enforces
the `concept → definitief → gearchiveerd` status lifecycle, confidentiality-based access
control via `vertrouwelijkheidaanduiding`, and supports drag-and-drop upload, full-text search,
version history, bulk operations, and ZIP export with `manifest.csv`.

**Tender demand:** 80% of analysed Dutch government tenders require structured case dossier
management; 65% explicitly demand ZGW DRC compliance.
