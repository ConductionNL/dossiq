...
Authoritative BRK (Basisregistratie Kadaster) parcel/ownership-reference lookup seam AND
authoritative WOZ (Waardering Onroerende Zaken) property-valuation lookup seam for Procest,
mirroring the existing BAG/BRP/KvK config-tier adapter pattern
(`external-integrations-test-environments`). Completes Procest's Dutch base-registration coverage
(BRP, BAG, BRK, HR, WOZ) for VTH (Vergunningen, Toezicht en Handhaving) and tax/spatial case
lifecycles.
