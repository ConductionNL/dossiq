...
**Scope:** procest
**Tier:** V1
**Depends on:** procest's existing canonical nav mechanism — `src/menu-layout.json` plus
`applyMenuRelocations`/`applyMenuRemovals` in `src/main.js` (ADR-037). No cross-app dependency.

## Purpose

Define the procest-side navigation contract that all configuration / admin surfaces live under a
single **Settings group** (via `src/menu-layout.json` relocations), that operational case surfaces —
including live fee calculations and the Termijnbewaking analytics dashboard — stay in the working nav,
and that every relocated page remains routable. This is the docudesk IA model (ADR-022) applied to
procest's mixed nav, expressed through ADR-037's canonical layout file, with no duplication of any
existing capability (ADR-012).

## ADDED Requirements

### Requirement: REQ-PCTS-001 — Configuration Surfaces Live Under A Single Settings Group

procest SHALL render every configuration / admin navigation leaf as a child of a single
...
- **GIVEN** procest's nav is built (manifest + fragments merged, `menu-layout.json` applied)
...
procest SHALL relocate the leges *tariff-admin* fragment leaf `LegesVerordeningen`
...
procest SHALL keep operational surfaces in the working (non-Settings) navigation and SHALL NOT
...
procest SHALL NOT change any page `id`, `route`, `type` or `component` as part of this relocation; the
...
SHALL only relocate existing menu leaves and correct one `section` flag. It SHALL reuse procest's
