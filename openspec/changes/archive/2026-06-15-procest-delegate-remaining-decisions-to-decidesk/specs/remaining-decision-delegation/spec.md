...
the merged `procest-delegate-contract-decision` (reuses its delegation + Besluit-materialisation
plumbing), and ADR-019 integration registry (OpenRegister side).

## Purpose

Define the procest-side contract for delegating the **remaining** decision/advice flows to
**decidesk** via the ADR-019 integration registry: the beslissing-op-bezwaar **backend**, the
BAC-adviezen (objection advisory committee advice), the advies/consultatie surfaces, and the
voorstel→besluit registration. Each raises a decidesk `Decision` of the appropriate `decisionType`
(ADR-005) and consumes the OUTCOME (fail CLOSED). procest keeps ZGW **case management** and records
the ZGW `Besluit`/advice record on the case as a **PROJECTION** of the decidesk outcome, with the
Awb domain rules retained as procest validation. This closes the decision/advice slices left after
the merged `procest-delegate-contract-decision` (ADR-012 dedup, ADR-022 consume-don't-duplicate,
cross-app contract #1) without breaking ZGW compliance.
...
### Requirement: REQ-PDRD-007 — Remove The Remaining procest-Local Decision/Advice Authoring Authority

procest SHALL remove its remaining parallel decision/advice **authoring** authority: the
besluit-authoring path in `Bezwaar/DecisionService` (`draft`/`publish` writing the besluit), the
BAC advice-outcome authoring in `Bezwaar/AdvisoryCommitteeService`, the advice-outcome authoring in
`AdviceService`/`ConsultationService`, and the direct besluit-registration authoring on voorstellen.
The case/zaak engine, the ZGW `Besluit` *recording* (as a projection), the Awb/IDOR/panel domain
rules, the parafeerroute, and the nav are NOT removed by this change.

#### Scenario: No procest-local path authors a remaining decision/advice

- **GIVEN** this change has shipped
- **WHEN** a beslissing-op-bezwaar, BAC-advies, advies/consultatie outcome, or voorstel-besluit is decided
- **THEN** the decision/advice SHALL be made by a decidesk `Decision`
- **AND** no procest-local decision/advice authoring state machine SHALL make it
- **AND** procest SHALL retain only the ZGW `Besluit`/advice *recording* (projection) + case orchestration + the domain validation rules
