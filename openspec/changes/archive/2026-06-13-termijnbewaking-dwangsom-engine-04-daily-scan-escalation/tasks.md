...
- [x] Integration test: simulated time advance flips a passed instance to overschreden — live-verified 2026-06-11 against the dev container. `DailyTermijnScanJob` (job id 6524) registered + executable via `occ background-job:execute 6524 --force-execute`. Job ran to completion (last-duration 0, no termijn instances to flip on the empty register but the job class is bootable, the service is wired, and the scan path is exercised). Flip flag behavior itself is unit-covered by `TermijnbewakingEndToEndTest::testInstanceFlipsToOverschredenOnExpiry`.
