...
## Purpose
Bridges procest's WOO besluit pipeline to OpenCatalogi's publication model so
an assembled WOO decision can be published to a public reading room, with
disclosure-safe document selection, DIWOO category mapping, status sync, and
graceful degradation when OpenCatalogi is not installed.
