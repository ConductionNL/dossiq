...
Authoritative BAG (Basisregistratie Adressen en Gebouwen) address and pand/verblijfsobject lookup
seam for Procest, mirroring the existing BRP/KvK config-tier adapter pattern
(`external-integrations-test-environments`). Fills the postcode+huisnummer structured-search gap
that neither `PdokBagService` (id-only) nor `PdokLocatieserverService` (fuzzy free-text) cover,
and provides an authoritative alternative to PDOK's open-data BAG mirror for cases where
provenance matters (VTH enforcement decisions).
