...
Enable and configure four already-merged nc-vue component-library features — multi-column
sort, notes `@mention`, saved views, and version history — on procest's case-management
surfaces, per ADR-Leaf-First (procest enables/configures shared components; it does not
reimplement them).
