# Spec: procest-procurement-supplier-management

**Status:** proposed
**Scope:** procest
**Tier:** procurement-suite
**Depends on:** case-management, case-types, roles-decisions, openregister (RBAC + audit + lifecycle + relations per ADR-022), docudesk (certificates + supplier-uploaded documents), openconnector (supplier portal + KvK/RGS/Peppol lookups)
...
HTTP surface; procest adds no per-app `SupplierController` for
basic supplier CRUD.
...
| `primaryContact` | string | No | UUID reference to a `contact` (procest existing register) |
...
| `onboardingCaseId` | string | No | UUID of the `Case` (procest `caseType: supplier-onboarding`) currently progressing this supplier |
...
- **GIVEN** procest is installed and the `Supplier` schema is loaded
...
type inherits every behaviour from procest's existing
...
- standard procest fields (`assignee`, `priority`, `deadline`)
...
- **GIVEN** a procurement officer is assigned to a supplier-onboarding
  case
- **WHEN** they open the procest my-work dashboard (existing
  `my-work` capability)
...
- **WHEN** the operator opens the case detail page (rendered by
  procest's existing case-detail renderer)
...
Per ADR-031 anti-pattern list, procest MUST NOT author a
...
Procest MUST NOT author a `SupplierPerformanceService` that loops
...
| `complaintCount` | aggregation over procest `customerContact` filtered by `supplier == self.id` |
...
accounts in a dedicated user-group (`procest-supplier-portal`) and
their per-supplier scope MUST be declared via OR's per-object RBAC
(ADR-022 row "Authorization RBAC"). Procest MUST NOT define a
...
- **GIVEN** a Nextcloud user in group `procest-supplier-portal` linked
  to supplier `S1`
...
- **THEN** OR's RBAC MUST return 403; no procest-side guard runs.
...
Procest MUST NOT author a `SupplierNotificationService` — per ADR-031
...
- a navigation entry `Procurement > Supplier onboarding` filtered by
  `caseType: supplier-onboarding`, reusing procest's existing case
  index renderer;
...
renderers per ADR-024 Tier-4. Procest MUST NOT author a per-page
...
- **WHEN** a `procurement-officer` opens `/index.php/apps/procest/
  suppliers`
