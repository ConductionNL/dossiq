...
(decision-flow delegation for the "Beslissingen op bezwaar" / "BAC-adviezen" surfaces — independent
concern). ADR-037 (modular config fragments + canonical `src/menu-layout.json` layout), ADR-012
(deduplication).

## Purpose

Define the procest-side information-architecture requirement that the six objections-and-appeals
(bezwaar & beroep) navigation surfaces — Bezwaren, Beroepen, Beslissingen op bezwaar, BAC-adviezen,
Bezwaaradviescommissies, plus the existing "Bezwaar & Beroep" group header — present as **one
coherent top-level group** instead of flat siblings, while every underlying page stays routable.
This is a pure nav-grouping change expressed through the existing `src/menu-layout.json` relocation
machinery (ADR-037); it adds no schema, controller, route, or page, and it does NOT delegate any
decision flow (that is the sibling change's concern).
