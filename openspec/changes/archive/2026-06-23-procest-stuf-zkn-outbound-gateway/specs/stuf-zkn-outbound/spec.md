...
# Spec: stuf-zkn-outbound

**Status:** proposed
**Scope:** procest (owns the StUF-ZKN/BG outbound gateway)
**Standard:** StUF 0310 (StUF-ZKN/BG), SOAP 1.1, WS-Security UsernameToken

## Purpose

Procest pushes zaakgericht-werken state to legacy zaaksystemen over StUF-ZKN/BG
0310. This capability is ported from pipelinq's production-grade outbound
engine and re-homed where zaakgericht-werken belongs. It builds SOAP envelopes,
transports them over mutually-authenticated HTTPS, parses the responses,
maintains an append-only audit log and a bidirectional case↔zaak mapping, and
isolates failing endpoints behind a circuit breaker with backoff retries.
