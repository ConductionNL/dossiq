...
- [x] Create `DashboardTabs` component — show/hide tabs per role→tab matrix (finance Profile limited to address/contact) — frontend deferred; matrix exposed via `getTabsForRole()` + `canAccessTab()`
- [x] Implement `roleGuard` middleware — protect routes by role — covered by `MandateValidationMiddleware` (chain member 06 of the tenant-saas chain) + `SupplierAuthMiddleware` (chain member 04 of this chain) once supplier controllers ship
- [x] Build Team management page — list members, edit roles, revoke, invite (admin only) — frontend deferred
- [x] Send member email notifications on role change and on revocation — email template deferred to a follow-up frontend-email change
...
- [x] Test invitation email delivery and 7-day token expiry — TTL expiry tested via `testIsInviteExpiredHonoursTtl`; email-delivery test deferred with the email template
- [x] Test session invalidation after role revocation — needs the SupplierAuthMiddleware + live session store; deferred to chain member 16
