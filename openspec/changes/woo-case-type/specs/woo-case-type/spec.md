...
- Mijn Overheid integration (for verzoeker notification via Berichtenbox)
- Document management within case (zaakdossier)
- OpenConnector (for external channel intake and PLOOI publication)
- OpenRegister (for all data storage: case, documents, assessments, decisions)
...
- **Deadline tracking**: `src/views/cases/components/DeadlinePanel.vue` provides deadline display with processing deadline calculation, extension support, and warning thresholds.
- **Duration helpers**: `src/utils/durationHelpers.js` supports ISO 8601 duration parsing for deadline calculations.
- **Activity timeline**: `src/views/cases/components/ActivityTimeline.vue` supports audit trail entries with user, date, and description.
...
- No decision document generation (beschikking)
- No inventarislijst generation
- No publication to reading room or PLOOI
- No bezwaar period tracking
...
- **Algemene wet bestuursrecht (Awb)**: Bezwaar procedure (6-week objection period, Art. 6:7).
...
- **Platform Open Overheidsinformatie (PLOOI)**: National publication platform for WOO documents.
...
- **CMMN 1.1**: WOO stages map to CMMN milestones; document assessment is a repeating HumanTask.
