...
- [~] P-1.1 In `lib/Settings/procest_register.json`, add `ncGroupId` as a nullable string — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-2.1 Add an `authorization` block to the `workflowStep` schema in `procest_register.json`. — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-2.2 Verify the `workflowTemplate` schema does NOT need a restrictive `authorization` — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-3.1 In the case type admin settings panel, add a text input field "NC Group ID" — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-3.2 In `StepConfigPanel.vue`, add a read-only display field "NC Group:" showing — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-4.1 Read `lib/Listener/KpiCacheInvalidationListener.php` and confirm: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-5.1 Write an integration test (Newman or PHPUnit integration) that: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-6.1 Write a PHPUnit unit test for the routing resolution logic: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-7.1 Add a note to `openspec/specs/role-based-step-routing/spec.md` in the `## ADDED — deferred to downstream cycle / fleet-wide adoption (handoff)
