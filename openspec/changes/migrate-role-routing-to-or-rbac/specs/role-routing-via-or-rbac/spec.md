...
Bring procest's role-based step routing into full compliance with OR RBAC (rbac-scopes +
auth-system). Step routing MUST produce Nextcloud group IDs that OR's RBAC stack enforces
at the data layer. The computation "which group handles this step" remains app-side; the
enforcement "only that group may access this object" MUST be delegated to OR. References
`consume-or-rbac-fleet-wide` umbrella and ADR-022 / ADR-023.
