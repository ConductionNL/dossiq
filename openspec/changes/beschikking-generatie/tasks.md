...
- [ ] **D01**: Confirm no `Beschikking`, `StateMachineLog`, `BezwaarTrigger`, or `MandaatRegeling` entities already exist in Procest. Confirm `procest_register.json` does not contain these schemas. Confirm that Docudesk template-engine supports PDF/A-3 output. Confirm that OpenConnector has eIDAS-TSP adapter support (KPN, EvidosSign, or similar). Confirm that OpenRegister supports TMLO-1.2 and MDTO metadata ingestion.

---

## Data Model (Procest Register)

- [ ] **T01**: Define `Beschikking` entity in `lib/Settings/procest_register.json` with all properties per design.md: id, zaakId, zaaktype, beschikkingType, kenmerk, templateId, huidigeStatus, geadresseerde (nested object), beslissing (nested), motivering, rechtsmiddelenClausule, legesbedrag, bekendmakingDatum, bezwaarTermijnEindDatum, mandaatGegeven (nested), handtekening (nested), verzending (nested), archief (nested), and seed data (3 example beschikkingen: wmo-toekenning-gearchiveerd, omgevingsvergunning-ondertekend, subsidie-ontwerp).

- [ ] **T02**: Define `StateMachineLog` entity in `procest_register.json` with properties: beschikkingId, overgang (nested: van, naar, tijdstip, actor, actorType, trigger, bewijsMateriaal).

- [ ] **T03**: Define `BezwaarTrigger` entity with properties: beschikkingId, bekendmakingDatum, bezwaarTermijnEindDatum, herinneringDatum, bezwaarOntvangen (boolean), bezwaarZaakId, archiefTriggerActief, archiefDatum.

- [ ] **T04**: Define `MandaatRegeling` entity with properties: id, naam, verleendDoor, verleendDatum, intrekkingsDatum, mandaatGroepen (array: niveau, tot_bedrag, zaaktypes, beschikkingTypes), ondermandaatToegestaan. Include seed data for at least one WMO mandaatregeling.

---

## API Endpoints (Procest)

- [ ] **T05**: Create `POST /api/beschikkingen` endpoint (Composition):
...
- [ ] **T06**: Create `GET /api/beschikkingen/{id}` endpoint (Read):
  - Return the full Beschikking object with all nested data

- [ ] **T07**: Create `PATCH /api/beschikkingen/{id}/akkoord` endpoint (Mandaat Approval):
...
- [ ] **T08**: Create `PATCH /api/beschikkingen/{id}/onderteken` endpoint (TSP Signing):
...
- [ ] **T09**: Create `PATCH /api/beschikkingen/{id}/verzend` endpoint (Berichtenbox Delivery):
...
- [ ] **T10**: Create `GET /api/beschikkingen/{id}/audit-pakket` endpoint (Audit Export):
...
- [ ] **T11**: Create `PATCH /api/beschikkingen/{id}` endpoint (Field Edit in Ontwerp Status):
...
- [ ] **T12**: Create `App/Jobs/BezwaarTermijnJob.php` (daily scheduled task):
...
- [ ] **T13**: Create `App/Jobs/ArchivalJob.php` (triggered by BezwaarTermijnJob or manual):
...
- [ ] **T14**: Create `App/Service/BeschikkingService.php` with public methods:
...
- [ ] **T15**: Create `App/Service/BerichtenboxRoutingService.php` with:
...
- [ ] **T16**: Create `App/Service/StateMachineService.php` with:
...
- [ ] **T17**: Create database migration to add Procest tables (or verify OpenRegister is used):
...
- [ ] **T18**: Create repair step `lib/Migration/SeedBeschikkingen.php`:
  - Loads `lib/Settings/seed/beschikkingen.json`
  - Creates three seed Beschikking objects in different states (gearchiveerd, ondertekend, ontwerp)
  - Creates corresponding StateMachineLog entries for each state transition
  - Idempotent (skip if beschikkingen already exist)

---

## Frontend: Composition UI

- [ ] **T19**: Create Vue component `src/views/case/components/BeschikkingComposerModal.vue`:
...
- [ ] **T20**: Create Vue component `src/views/case/components/BeschikkingDetailView.vue`:
...
- [ ] **T21**: Create Vue component `src/views/case/components/BeschikkingActionBar.vue`:
...
- [ ] **T22**: Create `src/services/beschikkingApi.js` with methods:
...
- [ ] **T23**: (OpenConnector) Implement eIDAS-TSP adapter:
...
- [ ] **T27**: Write integration tests for the full beschikking lifecycle:
...
- [ ] **T30**: Document the beschikking pipeline in README:
...
- [ ] **V01**: Full lifecycle test — compose → akkoord → sign → deliver → archive — runs successfully with seed data

- [ ] **V02**: Immutability test — attempt to edit ondertekend beschikking fields → HTTP 409 rejection

- [ ] **V03**: Mandaat test — attempt to approve with insufficient authorization level → HTTP 403 rejection

- [ ] **V04**: Audit-pakket test — export audit-pakket → verify ZIP signature → verify TSP report inside

- [ ] **V05**: State-machine test — attempt invalid transition (e.g., verzonden → ontwerp) → HTTP 409 rejection

- [ ] **V06**: Seed data test — fresh install → repair step creates 3 seed beschikkingen in correct states

- [ ] **V07**: Template versioning test — compose with known bekendmakingDatum → verify correct template version used

- [ ] **V08**: Bezwaar-trigger test — transition beschikking to verzonden → verify BezwaarTrigger created with correct dates

- [ ] **V09**: Archival test — run BezwaarTermijnJob when bezwaarTermijnEindDatum reached → verify ArchivalJob called → verify beschikking state = gearchiveerd + archief metadata recorded
