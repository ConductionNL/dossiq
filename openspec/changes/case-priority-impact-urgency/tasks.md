...
- [ ] 4.1 Declare an order and an NL Design System colour token per
  priority value; add the sortable column to `src/manifest.json` (D-7).
  - `tests/vitest/caseListPriority.spec.js`
- [ ] 4.2 Consume nextcloud-vue's list rendering for the order and the
  colour; ship no colour in a dossiq component (D-7).
- [ ] 5.1 `lib/Service/DeadlineEscalationService.php`: read the case's
...
- [ ] 6.1 Dutch and English strings for impact, urgency, priority and the
  override reason.
...
- [ ] 6.3 `tests/e2e/case-priority.spec.ts`: derive, override, clear,
