...
- [x] 1.1 `lib/Settings/dossiq_register.json`, schema `case`: declare
...
- [x] 1.2 `lib/Settings/dossiq_register.json`, schema `complaint`: the same
  annotation on `complaintNumber` with its own counter per D-5, and move that
  schema's version too.
- [x] 1.3 Retire `ComplaintService::generateComplaintNumber`. The number comes
  back off the saved object, so the log line says what was issued rather than
  what was guessed.
  - `tests/Unit/Service/ComplaintServiceTest.php`
- [x] 1.4 `lib/Service/CaseNumberService.php`: the same backfill, its docblock
  naming the annotation it now hedges against per D-4.
- [x] 1.5 The case page shows the number as issued and refuses to take a new
  one. Already true before this change and now guarded rather than assumed
  (`tests/vitest/caseIdentityManifest.spec.js`): `identifier` stays `editable: false` in `case-core`, and a refused
  update surfaces OpenRegister's sentence per D-6.

## 2. The star

- [x] 2.1 `src/services/favouriteApi.js`: `PUT` and `DELETE` on
...
- [x] 2.2 `src/components/case/CaseFavouriteStrip.vue` plus the
  `case-favourite` widget type in `src/registry.js` and its cell on
  `#CaseDetail`, per D-8 and D-9.
- [x] 2.3 `src/utils/caseFavourite.js` and the `favourite` row action on
  `#Cases` and `#Queue`, registered in `src/customComponents.js`.
- [x] 2.4 `src/manifest.json`: the Favourites and Recently opened chips on
  `#Cases`, and the two tiles on `#Dashboard`, per D-10.
- [x] 2.5 `l10n/en.json` and `l10n/nl.json`: the new labels, sentence case.

## 3. Verification

- [x] 3.1 `tests/e2e/case-number-and-favourites.spec.ts`: written and tagged.
  NOT RUN. No Playwright in this lane, so the file is a declaration of what
  the scenarios expect and not evidence that they hold. `npx tsc --noEmit` on
  it exits 0, which says it compiles and nothing more.
- [x] 3.2 The diff check, one `composer check:strict`, one `npm run lint`,
  `npm run test:l10n`. Exit codes read from `$?`, not from summaries.

## 4. Named back to openregister

These are RECORDED, not built. The box is ticked because recording them
is the task; each names what has to land in openregister first.

- [x] 4.1 A format per case type. `x-openregister-generated` has no
  ...
  what ships here.
- [x] 4.2 A second, human number beside the system one (C-case-core-20) and a
...
- [x] 4.3 A star on a list ROW needs no library support today because dossiq
