# Proposal: dossiq-delegation-via-events
...
delegation policy decided in `dossiq-delegate-contract-decision` and
`dossiq-delegate-remaining-decisions-to-decidesk` (both archived) — it FIXES the broken transport
...
event-based. Aligning dossiq onto the event contract is the minimal correct fix and removes the
...
1. **Dispatch via `IEventDispatcher`.** Inject the dispatcher, build `DecisionRequestedEvent` with
   positional/named ctor args (`sourceApp='dossiq'`, subjectRegister/Schema/Id/Label, decisionType,
...
2. **Listen via `DecisionConcludedEvent`.** Register `DecisionConcludedListener`; filter
   `getSourceApp()==='dossiq'`; map the event getters (status/outcome/decidedAt/signers/signing) to the
...
- [x] Project: `dossiq` — all implementation tasks are in this repo.
- [x] Project: `decidesk` — provides the merged event contract (`DecisionRequestedEvent` /
  `DecisionConcludedEvent`); no dossiq-side code in decidesk.
...
- A `DecisionConcludedEvent` with `getSourceApp()==='dossiq'` materialises the ZGW `Besluit` from the
