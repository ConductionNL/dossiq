...
- [ ] 1.1 `register.d/40-kcc-werkplek.json` and `dossiq_mock_register.json`:
...
- [ ] 1.2 `CitizenLookupGuard::redactForCaller()`: remove those four keys from
...
- [ ] 4.1 `CitizenLookupGuard`: the class comment stops claiming a field
...
- [ ] 5.1 `tests/e2e/citizen-lookup.spec.ts`: three principals, per D-5.
- [ ] 5.2 `openspec validate citizen-lookup-is-guarded-and-recorded --strict`.
