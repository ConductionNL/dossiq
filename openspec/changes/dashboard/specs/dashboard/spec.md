...
### REQ-DASH-003 (IMPLEMENTED) — Cases by Type Chart [V1]

Added `CaseTypeChart.vue`: horizontal CSS bar chart of open cases grouped by case type, sorted by count descending. Click on a bar navigates to the Cases view filtered by that case type. Follows the same CSS bar chart pattern as the Cases by Status chart.
...
### REQ-DASH-010 (IMPLEMENTED) — Doorlooptijd Navigation Link [V1]

Added a "Analytics" navigation item in the left sidebar (MainMenu.vue) pointing to `/dashboard/analytics`. The main dashboard header also displays a "View Analytics" link that navigates to the same route.
...
### REQ-DASH-V1-001 — Signalering: Deadline Alerts Widget [V1]

`DeadlineAlertsWidget.vue` displays cases approaching or past their processing deadline. Warning threshold: 3 days (configurable via `IAppConfig` in V2). Integrates `getDeadlineAlerts()` from `signaleringHelpers.js`.
...
### REQ-DASH-V1-002 — Signalering: Task Due Reminders Widget [V1]

`TaskDueRemindersWidget.vue` shows the current user's tasks with due dates within 3 days or past due. Integrates `getTaskReminders()` from `signaleringHelpers.js`. Tasks without a `dueDate` are excluded.
...
### REQ-DASH-V1-003 — Signalering: Stalled Cases Widget [V1]

`StalledCasesWidget.vue` identifies open cases with no `updatedAt` change in 7+ calendar days. Integrates `getStalledCases()` from `signaleringHelpers.js`.
...
### REQ-DASH-V1-004 — Woo Deadline Tracking Panel [V1]

`WooDeadlinePanel.vue` lists open cases whose case type title contains "Woo" (case-insensitive). Displays statutory deadline countdown with traffic-light severity. Woo responses are due within 4 weeks (28 days) with a single 2-week (14-day) extension. Integrates `getWooCases()` from `signaleringHelpers.js`.
...
### REQ-DASH-V1-005 — Process Analytics View [V1]

`ProcessAnalytics.vue` provides SLA compliance analytics at `/dashboard/analytics`. Implements the SLA compliance and throughput requirements from `openspec/specs/doorlooptijd-dashboard/spec.md`.
...
### REQ-DASH-V1-006 — Workflow Board View [V1]

`WorkflowBoard.vue` at `/workflow-board` provides a Kanban board with one column per non-final status type, case cards in each column, and drag-to-advance status transition.
...
### REQ-DASH-FIX-001 — Application.php Widget Registration [FIX]

Registers the three existing Nextcloud Dashboard widget classes (`CasesOverviewWidget`, `MyTasksWidget`, `OverdueCasesWidget`) in `Application.php` via `$context->registerDashboardWidget()`. Fixes `CasesOverviewWidget`'s route reference from the non-existent `.dashboard.index` to the correct `.dashboard.page`.
