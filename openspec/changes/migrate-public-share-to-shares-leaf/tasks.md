...
> **Build status (hydra audit 2026-06-10).** Spec is leaf-adoption-only; gated on the OR shares leaf shipping (BLOCKED_EXTERNAL pattern). All tasks are pre-migration verification + post-leaf-ship implementation. Tasks deferred until the leaf is live.
