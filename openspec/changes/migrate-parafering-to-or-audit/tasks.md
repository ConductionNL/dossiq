...
- [~] P-1.1 Update the constructor of `lib/Listener/ParaferingAuditListener.php` to inject — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-2.1 Implement the `handle()` method to: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-2.2 Remove the old `ObjectService::saveObject()` call and any `paraferingAuditEntry` — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-3.1 Delete `lib/Validator/ParaferingAuditAppendOnlyValidator.php`. — deferred to downstream cycle / fleet-wide adoption (handoff)
  - **Acceptance:** File does not exist in the repo after this task.

- [~] P-3.2 Remove the three event listener registrations for `ParaferingAuditAppendOnlyValidator` — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-4.1 Add `"deprecated": true` and `"deprecationNote"` to the `paraferingAuditEntry` — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-5.1 Write a PHPUnit unit test for `ParaferingAuditListener::handle()`: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-5.2 Write an integration test (Newman or PHPUnit integration) that: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-6.1 Update the existing `parafering-audit-trail` spec to reference the new consumer — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] P-7.1 Add an entry to `CHANGELOG.md` (or equivalent) noting: — deferred to downstream cycle / fleet-wide adoption (handoff)
