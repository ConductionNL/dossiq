...
- [ ] 1.1 `caseType`: declare the acknowledgement, the intake channels
...
- [ ] 1.2 `lib/Listener/AcknowledgementOnCreateListener.php`: a listener
...
- [ ] 1.3 Refuse to publish a case type whose statutory entry was removed
  without a warning naming Awb 4:3a (D-9).
- [ ] 2.1 `lib/Settings/templates/ontvangstbevestiging.json` and
...
- [ ] 2.2 Read the citizen-visibility flag from the same place the portal
  reads it, so nothing is quoted back that the citizen may not see (D-5).
- [ ] 3.1 Route through the citizen's recorded channel where there is one,
  and the case type's default otherwise, over openregister's notification
  dialect (ADR-031) (D-7).
- [ ] 3.2 Content-on-the-platform delivery: the message says one is
  waiting and carries no case content (D-6).
- [ ] 4.1 Write the outbound communication record on the case: moment,
  channel, recipient, template version (D-8).
  - `tests/vitest/caseCommunicationRecord.spec.js`
- [ ] 4.2 An unmet acknowledgement shows on the case, is retried, and can
  be recorded as met another way, with who said so (D-3).
  - `tests/unit/Service/AcknowledgementDutyTest.php`
- [ ] 5.1 The declared list of other automatic moments, with the request
  message distinct from the status-change message (D-9).
- [ ] 5.2 Dutch and English strings; hand portaliq the administered portal
  notice half of C-communication-32, with the candidate id.
- [ ] 5.3 `tests/e2e/ontvangstbevestiging.spec.ts`: a portal aanvraag, a
