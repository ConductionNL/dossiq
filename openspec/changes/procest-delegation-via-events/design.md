# Design — procest delegation via IEventDispatcher events

## Context

`procest-delegate-contract-decision` + `procest-delegate-remaining-decisions-to-decidesk` (archived)
...
**Dispatch (procest → decidesk), synchronous in-process via `IEventDispatcher::dispatchTyped()`:**
...
**Listen (decidesk → procest) for the terminal outcome:**
...
- Register via `$context->registerEventListener(DecisionConcludedEvent::class, DecisionConcludedListener::class)`;
  filter `if ($event->getSourceApp() !== 'procest') return;` then project onto the domain record.

## Decisions

1. **Reference the event classes by string in the listener registration but by FQN in the dispatch
   guard.** procest already registers OpenRegister approval events by FQN string
   ...
   and fails closed when false. This keeps procest installable without decidesk.
...
- No path sets a procest-local "approved" state as a fallback.
