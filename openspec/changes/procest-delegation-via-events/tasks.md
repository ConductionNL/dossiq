# Tasks — Delegate procest decisions to decidesk via IEventDispatcher events

## Phase 1: Dispatch path (procest → decidesk)
...
## Phase 2: Listen path (decidesk → procest)

- [ ] Add `lib/Listener/DecisionConcludedListener.php` implementing `IEventListener`, filtering
      `getSourceApp() === 'procest'`, building the normalised outcome from the event getters, and
