# Tasks — spec-anchor-repair (procest)

- [x] task-1: Measure broken `@spec` anchors repo-wide with gate-46 resolution logic (2,873 on base `origin/development`).
...
- [x] task-7: Apply the repointer — 573 anchors repointed across 216 files (79 anchor-level, 494 file-level).
- [x] task-8: Verify 0 repoint candidates rejected by the gate-46 post-condition check.
- [x] task-9: Comment-only proof — 0 non-`@spec` changed lines out of 1,146; 0 files with asymmetric insertions/deletions; 0 files touched outside `lib/`/`src/`.
- [x] task-10: Reconcile the tool's count against gate-46 exactly (`before − repointed == after`).
- [x] task-11: Gate-46 re-verify — broken **2,873 → 2,300**.
- [x] task-12: File the 2,300 residual-dangling anchors for human triage (`residual-dangling.md` + umbrella issue).
