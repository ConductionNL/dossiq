...
- [~] **TASK-CT-00: Verify no overlap with existing platform capabilities** — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] In `src/store/store.js`, add `registerObjectType` calls for the 5 missing sub-entities — deferred to downstream cycle / fleet-wide adoption (handoff)
  ...
  ```
- [~] Verify type names are kebab-case (ADR-015) — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Verify no duplicate registrations exist across OBJECT_TYPES or ENTITY_STORES — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] In `lib/Settings/procest_register.json`, under the mock data `components.objects[]` section, add: — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] All objects use `@self` envelope with `{ "register": "procest", "schema": "...", "slug": "..." }` — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] slugs are unique, human-readable, and stable (used for idempotency on re-import) — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] All caseType objects pass the member-02 `validatePublish()` check (≥1 statusType, ≥1 isFinal, validFrom set) — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Values follow Dutch conventions: Dutch field values, valid ISO 8601 durations, realistic descriptions — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Re-run `importFromApp()` on an existing install: verify no duplicates created (idempotency test) — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] Add all new user-visible strings to `l10n/en.json` (key == value, English) — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Add Dutch translations for all new strings to `l10n/nl.json` — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Strings to add (at minimum): — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] Verify zero gaps between en.json and nl.json key sets — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] Fresh install: run repair step → verify 4 case types appear with all sub-entities in admin UI — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Re-run repair step → verify no duplicates — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Browser console: confirm the 5 sub-entity stores resolve without error on app load — deferred to downstream cycle / fleet-wide adoption (handoff)
