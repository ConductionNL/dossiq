...
- [x] 1.1 Case store: `liveUpdatesPlugin` for `@objectId` and its runs
...
- [x] 1.2 `src/manifest.json` `#CaseDetail` `case-flow-runs`: drop
  `pollSeconds`; vitest asserts none on the page (D-3).
- [x] 2.1 `tests/e2e/case-detail-kpis-and-tabs.spec.ts`: the two-session
