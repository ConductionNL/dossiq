...
- [~] TASK-ZMP-02: DEFERRED (live instance) — OpenConnector DigiD/eHerkenning/Machtigen/Ketenmachtiging wiring + trust-level mapping happens at the instance edge. The portal side is ready: `PortalIdentityService::assertTrustLevel()` enforces the Wdo "substantieel" minimum and `deriveSubjectRef()`/`maskBsn()` pseudonymise the BSN/KvK. Dormant adapter contracts SHIPPED 2026-06-11: `lib/Service/Auth/DigidSamlAdapterInterface.php` (+ `LogDigidSamlAdapter`) and `lib/Service/Auth/EHerkenningSamlAdapterInterface.php` (+ `LogEHerkenningSamlAdapter`), both wired in `lib/AppInfo/Application.php` and emitting `BrokerAssertionResult::forDigid(bsn,...)` / `forEHerkenning(kvkNummer,...)`. Swap the DI aliases to the active impl once the openconnector broker URLs + cert + private key are configured and the `digid.feature_flag` / `eherkenning.feature_flag` keys are flipped.
