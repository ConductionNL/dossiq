...
- [~] TASK-ZMP-02: DEFERRED (live instance) — OpenConnector DigiD/eHerkenning/Machtigen/Ketenmachtiging wiring + trust-level mapping happens at the instance edge. The portal side is ready: `PortalIdentityService::assertTrustLevel()` enforces the Wdo "substantieel" minimum and `deriveSubjectRef()`/`maskBsn()` pseudonymise the BSN/KvK. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-03: PARTIAL — `PortalIdentityService` resolves the authenticated portal subject and derives a salted, pseudonymous `subjectRef`. DEFERRED (live instance): JWT issuance + IP/user-agent binding + 15-min TTL refresh belong to the OpenConnector edge session, not the host app. — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-ZMP-13: DEFERRED (live instance) — OpenConnector admin configuration UI for DigiD/eHerkenning endpoints/credentials is an instance-edge concern; the host app reads `portaal_subject_salt` from app config. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-14: DEFERRED (live instance) — IP + user-agent session binding is enforced on the OpenConnector edge JWT, not the host app. The host app's `PortalAuditLogger` records security-relevant events. — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-ZMP-15: DEFERRED (OpenConnector edge) — JWT-refresh AuthLayout wrapper is an edge-session concern; the in-app pages run inside the authenticated NC shell. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-16: DEFERRED (OpenConnector edge) — DigiD/eHerkenning LoginPage + SSO redirect is handled at the edge before the app loads. — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-ZMP-20: PARTIAL — document ACL + audit logic implemented server-side (`PortalDocumentService`, TASK-ZMP-05). A standalone `DocumentList.vue` tab is a follow-up. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-21: PARTIAL — messaging persistence implemented server-side (`PortalMessageService`, TASK-ZMP-06). A standalone `MessagingWidget.vue` thread UI is a follow-up. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-22: PARTIAL — bezwaar validation/submission implemented server-side (`PortalRequestService`, TASK-ZMP-07). A standalone `BezwaarForm.vue` is a follow-up. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-23: PARTIAL — klacht validation/submission implemented server-side (TASK-ZMP-08). A standalone `KlachtForm.vue` is a follow-up. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-24: DEFERRED (cross-app) — subsidie listing requires the opencatalogi API; out of the host-app build scope. — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-ZMP-26: SUPERSEDED — manifest-v2 provides the app shell/menu; the portal adds menu entries via `src/manifest.d/50-zaakportaal.json`. No bespoke MainLayout/MainMenu (they do not exist in manifest-v2). — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-27: PARTIAL — loading + error + skip-link states are inline in the portal views (NcLoadingIcon, role="alert", skip link). A SessionWarning timer is a DEFERRED edge concern. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-28: SUPERSEDED — routing is declarative via the manifest `pages[]` (`/portaal/mijn-zaken`, `/portaal/notificaties`); manifest-v2 has no `src/router/index.js`. — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-ZMP-31: DEFERRED (content) — privacy notice + accessibility statement are tenant content/legal copy, produced outside the build. — deferred to downstream cycle / fleet-wide adoption (handoff)

## Integration & Testing

- [~] TASK-ZMP-32: DEFERRED (cross-app) — n8n notification/intake/audit workflows require the n8n instance and cross-app wiring; out of host-app build scope. The host app emits the audit records (`PortalAuditLogger`) and persists the objects these workflows would consume. — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] TASK-ZMP-35: DEFERRED (live instance) — integration tests against DigiD callback / live OpenRegister need a running instance. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-36: DEFERRED (live instance) — Playwright E2E needs the deployed portal + DigiD/eHerkenning mock. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-37: DEFERRED (live instance) — automated axe/manual SR accessibility audit needs the rendered pages on a live instance. Accessibility primitives are built in (TASK-ZMP-30). — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-38: DEFERRED (live instance) — penetration/security testing needs a running endpoint. Build-time controls in place: IDOR-safe subject scoping, ACL enforcement, no raw BSN logged, input validation, `#[NoAdminRequired]` auth on every endpoint. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-39: DEFERRED (live instance) — load/performance testing needs a populated running instance. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-40: DEFERRED (docs) — API/deployment/user/admin documentation is a separate docs deliverable. — deferred to downstream cycle / fleet-wide adoption (handoff)

## Deployment & Release

- [~] TASK-ZMP-41: DEFERRED (infra) — CI/CD pipeline is repo/infra configuration, not application code. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-42: DEFERRED (infra) — production environment (subdomain, HSTS, WAF, backups) is operations work. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-43: DEFERRED (live instance) — tenant admin configuration UI for DigiD/eHerkenning + templates + subsidy list is an edge/admin concern. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-44: DEFERRED (release) — soft launch / canary monitoring is a release activity. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] TASK-ZMP-45: DEFERRED (release) — citizen/staff communication and training is a release activity. — deferred to downstream cycle / fleet-wide adoption (handoff)
