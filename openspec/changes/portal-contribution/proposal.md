...
Contribute procest's supplier surface to the shared **Portaliq** external portal
(hydra ADR-046) instead of procest hosting its own supplier portal. procest ships
a single, dependency-free `OCA\Procest\Portal\PortalContributionProvider` that
...
- procest keeps ownership of its data + domain logic; it only *declares* what a
  supplier may see. No portal auth, shell, or inbox logic lives in procest.
...
- Retire procest's in-app supplier views (`src/views/leverancier/*`) once Portaliq
...
- procest's existing supplier schemas in OpenRegister (supplierTender/Contract/
