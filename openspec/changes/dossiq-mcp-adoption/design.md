...
Dossiq today ships `lib/Mcp/DossiqToolProvider.php` — a hand-written `IMcpToolProvider` with 2 read-only tools (`dossiq.listProcesses`, `dossiq.getProcessDetails`), registered via the AppHost `Bootstrap` `mcpProvider` key in `lib/AppInfo/Application.php`. ADR-063 (hydra #102) supersedes that model: OpenRegister derives `{appId}.{schema}.{verb}` tools from a per-schema `x-openregister-mcp` block, and genuine non-CRUD behaviour is exposed with `#[McpTool]` on a service method. Apps hand-write no MCP tool code.

Two facts from the OpenRegister contract (`origin/development`) shape every decision below:

1. **Hand-written beats derived.** `SchemaDerivedToolProvider` is appended *after* the app's hand-written provider and self-suppresses colliding ids (REQ-DERIVED-003). A surviving `dossiq.case.search`-equivalent hand-written tool would permanently shadow the derived one — the dialect would be inert. Surgery and dialect must land in the same change.
2. **The dialect has no server-side field projection.** `SchemaDerivedToolProvider::search()` returns whole serialized objects (long strings truncated); `get()` returns the object in full. `fields` is a *caller-supplied* argument, not a schema-declared allowlist. So the only levers Dossiq has over what an agent can see are: **which schemas are enabled**, **which verbs**, and **which search filters**. This is the whole reason the curation below is conservative.

Dossiq owns **155 schema slugs** — 83 in `dossiq_register.json`, 6 in `ori_register.json`, 66 across 18 `register.d/` fragments. Blanket exposure would emit ~775 tools. Specter's research measures ~9.5% LLM tool-selection accuracy degradation and 30k+ tokens of context burn from exactly this. Curation *is* the change.
...
|---|---|---|---|---|
| 1 | `case` (`dossiq_register.json`) | search, get | `status`, `caseType`, `assignee`, `priority`, `identifier`, `isFinalStatus` | The zaak. Every question in this domain ("what's the status of case X", "which cases are still open", "what's on my desk") resolves to it. `assignee` + `isFinalStatus` replace the deleted `listProcesses`. |
| 2 | `task` (`dossiq_register.json`) | search, get | `status`, `isTerminalStatus`, `case`, `assignee`, `dueDate`, `priority` | The taak — "what do I still have to do on this case", "what's overdue". Carries no citizen data (assignee is an NC user id). |
| 3 | `caseType` (`dossiq_register.json`) | search, get | `identifier`, `catalogus`, `isDraft` | The zaaktype blueprint: statutory `processingDeadline`, confidentiality default, initial status. Public administrative metadata; answers "how long may this take". |
| 4 | `statusType` (`dossiq_register.json`) | search, get | `caseType`, `isFinal` | `case.status` is a UUID ref; without `statusType.get` the agent cannot *name* a case's status. `search` by `caseType` answers "what are the phases of this process". |
| 5 | `statusRecord` (`dossiq_register.json`) | search | `case`, `statusType` | The transition history. This is exactly the history join the deleted `getProcessDetails` hand-rolled; OpenRegister sorts and pages it for free. |
| 6 | `decision` (`dossiq_register.json`) | search, get | `case`, `decisionType`, `decisionDate` | The besluit — the legally operative outcome of a case. Reference fields only, no embedded citizen data. |
| 7 | `result` (`dossiq_register.json`) | get | — | `case.result` is a UUID ref; `get` lets the agent resolve "what was the outcome". No `search`: a free-text scan of outcomes across all cases has no honest use. |
| 8 | `resultType` (`dossiq_register.json`) | search, get | `caseType`, `archivalAction` | Carries `archivalPeriod` / `archivalAction` / `selectionListClass` — the Archiefwet retention rules. Answers "how long do we keep this case, and is it destroyed or transferred". Read-only reference data. |
| 9 | `document` (`dossiq_register.json`) | search, get | `documentType`, `status`, `confidentiality` | The informatieobject — "which documents belong to this case, and are they definitief". Risk-noted below (`content`). |
| 10 | `caseDocument` (`dossiq_register.json`) | search | `case`, `document` | `document` has no `case` property — this link record is the *only* path from a case to its documents. Search-only; it is a pure join row. |
| 11 | `bezwaar` (`dossiq_register.json`) | search, get | `case`, `status`, `objection` | The AWB objection lifecycle with its statutory clocks (`decisionDeadline`, `verdaagdOp`, `opschorting`). Top-frequency question in this domain. References only — the citizen sits behind `objection`/`case`. |
...
| 13 | `complaint` (`dossiq_register.json`) | **get only** | — | The klacht (Awb ch. 9). `get` only, deliberately: `complaint.klager` is an **embedded citizen record** (naam + contact). No `search` ⇒ no fishing for complainants (§ AVG). |
...
|---|---|---|---|
| `dossiq.listProcesses(limit?, status?)` | **Derivable CRUD.** Body = `searchObjectsAsArrays(case, filters, _limit)` + a cap. Zero business logic. | **DELETE** | `dossiq.case.search` — same `status` filter plus 5 more, with real pagination (`page`/`pageSize`/`total`/`hasMore`) instead of a hard 20-item truncation. |
| `dossiq.getProcessDetails(id\|uuid)` | **Derivable CRUD (composite).** Body = `find(case)` + `searchObjectsAsArrays(statusRecord, case=uuid)` + a `usort` on `createdAt`. The join is a second query, not domain logic. | **DELETE** | `dossiq.case.get` + `dossiq.statusRecord.search(filters:{case})`. Two derived calls; OpenRegister sorts and pages. |

Both tools are derivable ⇒ **the provider retains zero tools ⇒ `lib/Mcp/DossiqToolProvider.php` is DELETED** (ADR-063: do not leave an empty seam). Consequently:

- No `#[McpTool]` method is added to any service in this change — nothing survived to move.
- **No `IMcpScannableServices` implementation is added.** The opt-in exists to tell OpenRegister *which services to scan*; with no `#[McpTool]` anywhere in Dossiq, a `DossiqScannableServices` returning `[]` would be a dead class. It gets added by the first change that introduces a curated tool.
- Also removed: the `'mcpProvider' => DossiqToolProvider::class` key + `use` in `lib/AppInfo/Application.php`, `tests/Unit/Mcp/DossiqToolProviderTest.php`, the `tests/Stubs/Mcp/IMcpToolProvider.php` stub with its `tests/bootstrap.php` require and its `psalm.xml` `referencedClass` suppression.

### D4 — Access model: the provider's ACL does not come back

`DossiqToolProvider::canReadCase()` allowed a read when the caller was a dossiq/NC admin, the case's `assignee`, or held a `role` record on the case. That ACL **exists nowhere else in Dossiq**: the Vue frontend reads cases straight from `/apps/openregister/api/objects` via `useObjectStore` (ADR-022 thin client), so the UI's effective access model already *is* OpenRegister RBAC. The provider's ACL was an MCP-only refinement.
...
- *Alternative considered:* keep a hand-written `dossiq.listMyCases` carrying the ACL. **Rejected** — it re-introduces a hand-written CRUD tool, which is exactly what shadows the derived surface (Rule 1), and it hides an ACL in the agent path that the human path does not have.
- *Mitigation:* the "cases I work on" question is served losslessly by `dossiq.case.search(filters:{assignee: <me>})`, which is why `assignee` is a declared filter.
- *Precondition:* Dossiq's register/schema RBAC must actually be configured before the dialect ships. A verification task covers it: a non-privileged user must not be able to `dossiq.case.search` the whole register.

### D5 — AVG / persoonsgegevens

Dossiq processes citizen data. With no server-side projection (§ Context, fact 2), what an enabled schema returns is *everything it stores*.
...
**Zero write verbs are declared on any Dossiq schema.** This is not caution-by-default; each refusal has a specific reason.
...
The general rule this expresses: **in Dossiq every lawful write goes through a service that enforces a guard, a mandate, a clock or a retention rule.** The derived write verbs, by construction, do none of that. The correct way to make a Dossiq write agent-safe is a curated `#[McpTool]` on the *owning service* (e.g. `StatusTransitionService::transition`, which already evaluates guards), with `scope: update`, `destructiveHint: false`, `readOnlyHint: false`. That is a follow-up change, deliberately not smuggled into this one (Q2).

## Risks / Trade-offs

- **[Stale hand-written tool shadows the derived surface]** → Provider deletion and dialect declaration ship in the *same* change; a verification task asserts `dossiq.listProcesses` is gone from `tools/list` and `dossiq.case.search` is present.
...
1. Add `configuration["x-openregister-mcp"]` to the 13 schemas (11 in `dossiq_register.json`, 1 in `register.d/60-termijnbewaking.json` — `complaint` and the rest are in the main register). `python3 -m json.tool` after every edit.
...
4. **Rollback:** revert the commit. The dialect is additive JSON; removing it removes the tools. Nothing in Dossiq's runtime depends on the derived tools existing.
...
- **Q2** — Should `StatusTransitionService::transition()` be exposed as a curated `#[McpTool]` (guard-enforcing, `scope: update`, human-approval-gated in Hermiq)? That is the only architecturally sound way an agent ever advances a Dossiq case. Needs a product decision, not an engineering one.
