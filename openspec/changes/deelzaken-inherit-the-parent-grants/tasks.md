...
- [ ] 1.1 `lib/Settings/dossiq_register.json`: declare `parentCase` as the
...
- [ ] 1.2 `lib/Service/CaseAccessGuard.php`: `hasCaseReadAccess()` and
...
- [ ] 1.3 Pin the verb rule: a read grant on a parent does not become a
  mutation grant on a deelzaak (D-3).
- [ ] 2.1 `src/manifest.json` `#CaseDetail`: a deelzaak opened through an
  inherited grant names the case that granted it (D-4).
  - `tests/vitest/caseAccessProvenance.spec.js`
- [ ] 2.2 The Sharing tab of a case with deelzaken states that a share
  reaches them (D-5), in Dutch and English.
- [ ] 3.1 `tests/e2e/deelzaken-inherit-the-parent-grants.spec.ts`;
