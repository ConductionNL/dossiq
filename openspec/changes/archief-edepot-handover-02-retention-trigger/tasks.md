...
- [~] Test: bezwaar case resumes to `gereed-voor-overdracht` after procedure ends — DEFERRED to live env; logic is asserted by reading `ArchivalTriggerService::resolveLegalProceedings` which re-queries on next nightly sweep
- [~] Test: blocked case produces a DIV notification (and optional task) — DEFERRED: covered behaviourally by the event dispatch; full live verification needs IEventDispatcher mock + procest notification stack
