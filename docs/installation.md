...
Dossiq ships with seed data containing example case types. To configure your own:

1. Open **Dossiq** from the left sidebar.
...
1. Go to **Admin settings** > **Basic settings** > **Dossiq**.
...
**Symptom**: Dossiq opens but lists show "No items found" and the admin settings report a missing register.
...
Or from the admin UI: **Admin settings** > **Dossiq** > **Re-import configuration**.
...
- **422 Unprocessable Entity**: A required ZGW field (e.g., `bronorganisatie`, `verantwoordelijkeOrganisatie`) is missing. Check the ZGW field mapping in **Admin settings** > **Dossiq** > **ZGW mapping**.
...
**Symptom**: Dossiq does not appear in the Nextcloud left sidebar.

**Fix**: Confirm the app is enabled for your account. An administrator can check this at **Admin settings** > **Apps** > **Dossiq**. If the app is restricted to a group, ensure your account is a member of that group.
