...
# Decision: Field inspections — keep in procest (it is domain, not data quality)
...
- loads the inspector's daily planning from local **IndexedDB** (synced via
  `GET /apps/procest/api/sync/daily`),
...
- **Is it needed:** yes — it is genuine procest domain functionality, distinct
...
- **Move to OpenRegister?** **No.** This is **not** data-quality monitoring
  ...
  It is a domain procedure with deeply procest-specific UX: offline IndexedDB
  ...
  workflow that produces them belongs in procest.
