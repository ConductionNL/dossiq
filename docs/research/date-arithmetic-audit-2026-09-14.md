...
| `lib/Service/Term/FirstResponseOutcome.php` | 243 | business | `WorkingDayCalculator` | the overrun on a first-response term (`term-configuration-beyond-the-case-type`). The `+1 day` is not a term end: it is where the count of days we were late STARTS, because the day we promised by is not a day we were late. The counting itself is the calculator's, so this file keeps no holiday list of its own. It moves to the engine calendar when one answers "how many working days between these two dates"; `WorkingDayRoll` rolls a single date and cannot answer that yet |
...
| `lib/Service/TermijnService.php` | 109 | statutory | engine calendar | `endDateCalculated`. Taken by `terms-on-the-engine-calendar`: `createTermijnInstance()` routes through `TermijnTimerService::rollTermEndFor()`, the one call every other term site makes, so the stored date and the armed timer come from one computation |
...
| `lib/Service/Custody/CaseTakeoverRequest.php` | 299 | business | | the answer period a handler has before a takeover request escalates. An internal service clock, not a term the applicant is owed, so it counts calendar days on purpose and reaches no calendar: the escalation is about how long the request has sat, and a weekend it sat through is time it sat |
