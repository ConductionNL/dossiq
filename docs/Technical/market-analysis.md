...
This page positions dossiq against the systems a municipality or an SMB would otherwise buy. It was first written before any competitor had been installed, and it read like it: every landscape table was a set of claims from feature pages. Since September 2026 the numbers on this page are measured. Thirty columns stand in the parity ledger, twenty-six of them installed and driven, and every score below is a `yes` count on the 180 domain-neutral rows, as of ledger v10.1 and gap register v4.1 (2026-09-14). The method, the counting rule, the statutory split and the full ranking are on [competitor parity, September 2026](../research/competitor-parity-2026-09.md); the links behind every cell are on [competitor sources](../research/competitor-sources.md). A second sweep asked what these systems have that our list never thought to ask, and it is on [competitor discovery, September 2026](../research/competitor-discovery-2026-09.md).
...
| "Nextcloud Deck: Kanban board for tasks, not case management." | Measured at 37 of 180, level with Kanboard, and the first non-Dutch column to pass seven document rows, all of them Nextcloud's (`nextcloud-deck/round4/open-core.md`). Batch 12 found two more that reach seven and eight with document managers of their own, Tuleap CE and Huly. A card is not a case, which is eleven honest `no` answers. | The same table. |
...
One claim this page never made, and a buyer does. A depth study read what a kenmerk can actually do, 54 capabilities over eleven systems and dossiq, and dossiq comes eleventh of twelve. xxllnc Zaken scores 43, Valtimo 34 and Dimpact ZAC with OpenZaak Catalogi behind it 33, against dossiq's 21. The gap is concentrated rather than spread: dossiq scores zero `yes` on the thirteen field types, which is the half a demo is made of, and is competitive only on case-type structure, the half the ZGW model gave us. The ranking, the four groups and the second reader who recounted ZAC, Valtimo and OpenCase are on [competitor discovery, September 2026](../research/competitor-discovery-2026-09.md), from `_round4/discovery/casetype-configurability.md`.

What the discovery sweep left open, Ruben closed on 2026-09-14. Twenty-two decisions were taken, and six differ from what the file recommended: revive all five parked candidates, set the promotion bar on relevance rather than on passer count, put the archiving process in openregister rather than filinq, use openregister's existing soft delete rather than a new recycle state, give the mail account to Nextcloud Mail rather than integriq, and re-rate the twenty `not` candidates with an MKB lens because the product serves a broad market including MKB. Each decision and its reasoning is on [competitor discovery, September 2026](../research/competitor-discovery-2026-09.md).
...
Round 2 drove four Dutch systems against the 206-row matrix (`_round2/compare/M1-functionality.md`); round 3 read Dimpact ZAC against the same rows. The 180-row figures are from `_round4/compare/twenty-six-system-tally.md`.
...
Set by Ruben on 2026-09-12: issue trackers, help desks and boards are a third competitor family, closer to dossiq in daily use than a BPM engine. Twenty-two were installed and driven and four rated from documents (`_round4/compare/README.md`). The open-core column names the shape from the fourteen the round found; the result page explains each.
...
| Tuleap CE 17.5 | ALM suite | GPL-2.0 | driven | 65 | yes, the thirteenth shape: the open edition verifies an Ed25519 licence it can never hold, and nineteen Enterprise plugins are absent. The source stopped being public on 2026-09-13 |
| Huly v0.7.426 | issue and workspace | EPL-2.0 | driven | 60 | no gate at all, and the fourteenth shape: the vendor's own self-host compose runs 1 of 22 services and 6 of 14 pods, with no statement of what is missing |
...
Gitea and Forgejo are one product for every question in the matrix: 23 yes and 48 partial each, and 0 of 206 rows differing. Batch 12 closed the undriven set the candidate table had carried since round 3.

Three things the family says as a whole. A product built around a team's own work has no party, no document record, no decision and no term engine, which is 53 rows between four sections; the help desks do better than the boards because a help desk is built around somebody outside the organisation. Every non-Dutch column scored 0 of 7 on decisions until batch 12, and Tuleap and Huly score 1 each, both on the same row: an outcome list configured per type. Nobody outside the Dutch family scores a besluit, a decision letter, a Woo assessment or an objection surface. And copyleft proves nothing about what you can deploy: most of these trees are GPL, LGPL or AGPL, and among them sits every open-core shape a grep has found.
...
| gap | evidence |
|---|---|
| Decisions | 1 of 7 at best in every non-Dutch column, twenty-two driven and four documented, and 0 in twenty-four of the twenty-six. The one row Tuleap and Huly pass is an outcome list per type. The nearest decision object anywhere is OpenProject's `MeetingOutcome` with a `decision` kind and no approver, quorum, number, effective date or publication. |
| Dutch statutory rows | 26 rows, twenty-six non-Dutch systems, 676 cells, zero `yes`. Every catalogue searched by name: 0 apps for ZGW, and none of the seven names on Easy8's 1,434 pages. |
| Documents | The weakest area of the whole non-Dutch corpus, with one exception. Znuny and GitLab share two `yes` answers over 46 document rows; Kanboard and Vikunja the same; Frappe Helpdesk, Request Tracker, Gitea and Taiga score one each of 23. Huly scores 8 and Tuleap 7, both from a document manager of their own. |
| Easter | Nothing in twenty-six non-Dutch columns computes it. Frappe Helpdesk and Request Tracker agreed to the minute on a Dutch year and both landed a 2027 term on Tweede Paasdag. dossiq computes it, in `lib/Service/WorkingDayCalculator.php`. |
| The edition you can deploy | Pending row 11.27. Fourteen open-core shapes across the corpus; dossiq passes because there is no paid half. |
...
| 8.23 | The deadline engine reads the calendar the organisation administers | Nobody yet. dossiq is `partial`: 37 files under `lib/` do date arithmetic and five call `WorkingDayCalculator`, three of the rest being term paths by name. |
| sections 4, 9, 3, 11 | Documents 7 of 23, search 4 of 12, tasks 6 of 19, configuration 9 of 24 | xxllnc, GZAC, OTOBO, Odoo. |

The gap register puts these and the rest into **161 gaps** with one owner each, 34 on dossiq and 69 on openregister. 152 of them name the OpenSpec change that carries them, across 85 changes in nine repositories, and the nine that do not each say why: two deliberate no's on Microsoft integration, two more on openregister, four platform facts that belong to Nextcloud, and one proposal waiting for a driven `yes`. The [result page](../research/competitor-parity-2026-09.md#the-gap-register-counted-by-owner) has the counts by owner, and the umbrella the dossiq changes, [33 at first](https://github.com/ConductionNL/dossiq/pull/2643) and [five more](https://github.com/ConductionNL/dossiq/pull/2684).

Twelve pull requests are merged so far: dossiq [#2719](https://github.com/ConductionNL/dossiq/pull/2719), [#2721](https://github.com/ConductionNL/dossiq/pull/2721), [#2723](https://github.com/ConductionNL/dossiq/pull/2723), [#2729](https://github.com/ConductionNL/dossiq/pull/2729), [#2732](https://github.com/ConductionNL/dossiq/pull/2732), [#2744](https://github.com/ConductionNL/dossiq/pull/2744), [#2746](https://github.com/ConductionNL/dossiq/pull/2746) and [#2748](https://github.com/ConductionNL/dossiq/pull/2748); openregister [#3707](https://github.com/ConductionNL/openregister/pull/3707), [#3711](https://github.com/ConductionNL/openregister/pull/3711) and [#3714](https://github.com/ConductionNL/openregister/pull/3714); portaliq [#551](https://github.com/ConductionNL/portaliq/pull/551). Thirteen more lanes are in flight and none is merged.
...
| The calendar as an OpenRegister schema, with Easter computed | Measured against twenty-six non-Dutch columns. Three of dossiq's own term paths do not call it, which is proposed row 8.23. |
...
| Non-Dutch products outscore dossiq on the comparable half | OTOBO 78, Odoo 78, Znuny 73 and Frappe Helpdesk 72 against 71; Easy Redmine 86 and JSM 83 as documented ceilings | The gap register: 161 gaps, 103 of them size S, 152 carried by a named OpenSpec change |
...
| OpenRegister dependency | 69 of 161 gaps, and 280 of the 636 discovery candidates, are owned by openregister, every one of them now carried by a change | The [openregister umbrella](https://github.com/ConductionNL/openregister/pull/3688) carries them; dossiq keeps a half on each |
| Documents are the largest area gap against the Dutch rivals, and now against one non-Dutch column | 7 of 23 against xxllnc's 18, OpenCase's 13 and Huly's 8 | `documents-on-the-case` and the Files rows Deck already passes on the same host |
