...
This page gets a working Dossiq running on your own machine in two commands. You end with case files and process steps.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/dossiq/development/dossiq-compose.yaml
```
...
==> installing dossiq <version>
==> apps present: dossiq integriq openregister thematiq
```
...
```bash
curl -s http://localhost:8609/status.php
```
...
| What | Where |
| --- | --- |
| **Dossiq** | [http://localhost:8609/apps/dossiq/](http://localhost:8609/apps/dossiq/) |
| Admin interface | [http://localhost:8609](http://localhost:8609) — `admin` / `admin` |
...
| `dossiq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing dossiq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -u admin:admin "http://localhost:8609/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f dossiq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f dossiq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f dossiq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
