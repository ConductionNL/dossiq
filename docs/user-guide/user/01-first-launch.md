...
You are set up correctly when: the Procest dashboard renders without an error banner, the left navigation lists the entries above, and clicking **Cases** (or any other list) shows either rows or a clean *No cases found* state: not a load error.
...
| Lists load but **Add Case** opens a dialog with no form fields | The Procest register import is incomplete: an admin re-runs **Administration settings → Procest → Re-import configuration**. |
