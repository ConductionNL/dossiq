...
| `consultation-email-fanout` | Webhook van Dossiq | direct bij creatie | externe adviesinstantie informeren met secure link |
| `consultation-bottleneck-detection` | Schedule | dagelijks 08:00 Europe/Amsterdam | knelpunt-melding wanneer overdue-rate > 20 % |

All workflows authenticate to dossiq via HTTP Basic auth (the n8n credential must be created as type "HTTP Basic Auth" and referenced from each HTTP request node). The `OCS-APIRequest: true` header is set so Nextcloud accepts JSON without the web login redirect. Inkomende webhooks van Dossiq zijn beveiligd met een HMAC-SHA256 signature in `X-Procest-Signature`.
...
6. Markeer `consultation.lastWarningAt` bij T-5 en `consultation.escalatedAt` bij overdue, zodat Dossiq dezelfde dag niet dubbel waarschuwt.

### Recipient resolution

- **Internal bodies** → NC group resolved via `adviesinstantieId`.
- **External bodies** → the configured email is dispatched by the dossiq NotificationService side (n8n only enqueues the notification request).

### Dossiq notification contract — `/api/consultations/{id}/notify`
...
Wordt direct aangeroepen door Dossiq wanneer een consultation wordt aangemaakt voor een **externe** adviesinstantie (geen Nextcloud-account).

### Stappen

1. **Webhook trigger** — `POST /webhook/dossiq/consultation-created`, fired by `ConsultationService::createConsultation` when the resolved advisory body has `type === 'external'`.
...
5. Verstuur de mail en log de message-id terug naar Dossiq via **POST** `/api/consultations/{id}/audit` met `event: external-email-sent` (BIO-compliant audit trail).

### Inkomend payload van Dossiq
...
```json
{
  ...
  "responseUrl": "https://example.gemeente.nl/index.php/apps/dossiq/external/consultations/<token>",
  "attachments": [
...
- The `responseToken` is delivered ONCE by dossiq (stored as SHA-256 hash); the workflow does NOT log the plaintext token anywhere.
...
2. Maak een credential **HTTP Basic Auth** met de service account die `n8n-procest` uid heeft, en koppel die aan alle Dossiq HTTP-nodes.
...
- **Dossiq dashboard** — onder **Beheer → Dossiq → Integraties → n8n** verschijnt de laatste succesvolle uitvoeringstijd van elke workflow.
...
| Dubbele T-5 waarschuwingen | `lastWarningAt` is leeg / niet teruggeschreven | Controleer dat de `notify` POST status 204 teruggeeft; werk het object alleen bij in Dossiq, niet in n8n. |
...
| Secure response link werkt niet | Token verlopen of consultation afgesloten | Coördinator opent de consultation en kiest **Token regenereren**, daarna stuurt Dossiq een nieuwe e-mail. |
