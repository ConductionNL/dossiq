...
Drop `*.json` fragment files in this directory to extend the Procest
...
`procest_register.json` and conflict on merge. ADR-037 lets each build instead
add its registers/schemas as an isolated fragment file here, eliminating the
shared-file contention.

## How it works

`SettingsService::loadConfiguration()` reads `procest_register.json` (the base),
...
- Each fragment mirrors the top-level shape of `procest_register.json`
