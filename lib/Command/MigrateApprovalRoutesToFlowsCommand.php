 ...
 * {@see \OCA\Dossiq\Service\Parafeer\EndorsementRouteFlowMigrator}.
 *
 * @category Command
 ...
 * @spec openspec/changes/approval-routes-are-flows/specs/approval-routes-are-flows/spec.md
 */
class MigrateApprovalRoutesToFlowsCommand extends Command {
	/**
	 * Wire the command against the migrator.
	 *
	 * @param EndorsementRouteFlowMigrator $migrator The migrator.
	 * @param IUserManager $userManager Resolves the acting user.
	 */
	public function __construct(
		private readonly EndorsementRouteFlowMigrator $migrator,
	...
	protected function configure(): void {
		$this->setName(name: 'dossiq:approval-routes:migrate-to-flows')
			->setDescription(
				'Project Dossiq approval routes onto OpenRegister flows (idempotent). '
				. 'An approval route is a sequence of manual sign-offs before a decision may be '
				. 'attached to a case, which is a flow. The flows arrive DISABLED: the route still '
				. 'drives parafering, and enabling a projection without retiring its route would ask '
				. 'every approver twice. Use --dry-run first.'
			)
			->addOption(
				name: 'user',
	...
	private function report(array $summary, bool $dryRun, OutputInterface $output): int {
		...
		$prefix = 'dossiq:approval-routes:migrate-to-flows';
		if ($dryRun === true) {
