...
class PlannedFollowUpDocument {
	 ...
	 *
	 * Named here as well as on the calendar because callers reach for it
	 * through this class, which is the one they already hold.
	 *
	 * @var string
	 */
	public const RECURRENCE_NONE = PlannedSeriesCalendar::RECURRENCE_NONE;
	 ...
	 */
	public const RECURRENCE_MONTHS = PlannedSeriesCalendar::RECURRENCE_MONTHS;
	 ...
	 * Constructor.
	 *
	 * The calendar carries a default so this class stays newable without a
	 * container, which is what keeps its rules testable without an instance
	 * of anything else.
	 *
	 * @param PlannedSeriesCalendar $calendar Answers which days a recurrence fires on.
	 */
	public function __construct(
		private readonly PlannedSeriesCalendar $calendar = new PlannedSeriesCalendar(),
	) {
	}//end __construct()
	...
	public function recurrenceOf(string $recurrence): string {
		return $this->calendar->recurrenceOf(recurrence: $recurrence);
	}//end recurrenceOf()
	...
	public function markerOf(array $nodes, ?DateTimeImmutable $from = null): ?array {
		...
		return [
			...
			'date' => $this->calendar->nextDate(series: $series, cron: $cron, from: $from),
	...
	public function nextOccurrence(string $recurrence, DateTimeImmutable $anchor, DateTimeImmutable $from): ?DateTimeImmutable {
		return $this->calendar->nextOccurrence(recurrence: $recurrence, anchor: $anchor, from: $from);
	}//end nextOccurrence()
	...
	private function cronFor(DateTimeImmutable $due, string $recurrence): string {
		...
		if ($this->calendar->usesLastDay(recurrence: $recurrence, anchor: $due) === true) {
			$day = 'L';
		}

		$months = '*';
		if ($recurrence !== 'monthly') {
			$months = implode(',', $this->calendar->monthsOf(recurrence: $recurrence, anchor: $due));
		}
	...
	private function describe(
		...
		return sprintf(
			...
			$caseTypeId,
			$this->calendar->inWords(recurrence: $recurrence),
			$due->format('Y-m-d'),
			$caseId,
			$uid
		);
