...
class WmsWfsService
{
     ...
     *
     * Validation rules (REQ-WMS-1, REQ-WMS-3):
     *  - title, type, url, layerName are required
     *  - type must be WMS or WFS
     *  - opacity must be between 0.0 and 1.0
     *  - url must resolve to an entry on the GIS proxy allowlist
     *
     * @param array<string, mixed> $layer The layer object to validate
     *
    ...
    public function validateLayer(array $layer): array
    {
        ...
        if ($title === '') {
            $errors[] = ['field' => 'title', 'code' => 'wms.title_required', 'message' => 'Title is required'];
        }

        $type = strtoupper((string) ($layer['type'] ?? ''));
        if ($type !== 'WMS' && $type !== 'WFS') {
            $errors[] = ['field' => 'type', 'code' => 'wms.type_invalid', 'message' => 'Type must be WMS or WFS'];
        }

        $url = trim((string) ($layer['url'] ?? ''));
        if ($url === '') {
            $errors[] = ['field' => 'url', 'code' => 'wms.url_required', 'message' => 'URL is required'];
        }

        $layerName = trim((string) ($layer['layerName'] ?? ''));
        if ($layerName === '') {
            $errors[] = ['field' => 'layerName', 'code' => 'wms.layer_name_required', 'message' => 'Layer name is required'];
        }

        if (array_key_exists('opacity', $layer) === true && $layer['opacity'] !== null) {
            $opacity = (float) $layer['opacity'];
            if ($opacity < 0.0 || $opacity > 1.0) {
                $errors[] = ['field' => 'opacity', 'code' => 'wms.opacity_range', 'message' => 'Opacity must be between 0.0 and 1.0'];
            }
        }

        // Allowlist check is delegated to GisProxyService::isUrlAllowed() at request time.
        ...
        if ($url !== '' && $this->isUrlAllowed(url: $url) === false) {
            $errors[] = [
                ...
                'message' => 'URL is not on the GIS proxy allowlist',
            ];
        }
     ...
     *
     * Delegates to GisProxyService's allowlist by attempting a private mirror
     * of the same host-matching rules. Returns true when the host matches a
     * configured layer URL or a PDOK/Kadaster default.
     *
    ...
    private function isUrlAllowed(string $url): bool
    {
        ...
        if (str_contains($host, 'pdok.nl') === true
            || str_contains($host, 'kadaster.nl') === true
        ) {
            return true;
        }
        ...
        try {
            $objectService = $this->container->get('OCA\OpenRegister\Service\ObjectService');
            $schemaIds     = [
                (int) ($this->settingsService->getConfigValue('map_layer_schema') ?? 0),
                (int) ($this->settingsService->getConfigValue('wms_layer_schema') ?? 0),
            ];
            ...
            foreach ($schemaIds as $schemaId) {
                ...
                foreach ($layers as $layer) {
                    ...
                    $layerUrl    = (string) ($layerArr['url'] ?? '');
                    $parsedLayer = parse_url($layerUrl);
                    $layerHost   = (string) ($parsedLayer['host'] ?? '');
                    if ($layerHost !== '' && $layerHost === $host) {
                        return true;
                    }
                }
            }//end foreach
            ...
            $this->logger->warning(
                'WmsWfsService::isUrlAllowed failed',
