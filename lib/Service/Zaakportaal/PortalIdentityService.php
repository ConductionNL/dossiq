...
class PortalIdentityService
{
     ...
     * currentSubjectRef(); both throw OCSBadRequestException when no user
     * is authenticated, and both pseudonymise the resulting UID into a
     * subject reference. Use this alias from controller methods that
     * scope their data access by the returned subject reference - that
     * scoping is itself the IDOR guard, but the gate needs an
     * authorize*/require*/ensure* call site to pick it up.
     *
     * @return string The pseudonymous subject reference for the caller.
     *
     * @throws OCSBadRequestException When no authenticated subject is present.
