...
class CaseStatusStore {
	...
	/**
	 * Find a caseType's statusType by NAME.
	 *
	 * The inverse of {@see lookupStatusName()}, and the lookup a SHIPPED flow
	 * needs. A flow declared in the app's register cannot carry a statusType
	 * uuid: those are minted per installation, so a hard-coded one would be
	 * wrong everywhere except the machine it was written on. The flow names the
	 * status the way a person does — "in behandeling" — and this resolves it
	 * within the case's own type, which is also what stops it matching a
	 * same-named status belonging to some other case type.
	 *
	 * Comparison is case-insensitive and trimmed, because the name is authored
	 ...
	 *
	 * @return string The statusType UUID, or the empty string when there is no such status.
	 *
	 * @spec openspec/changes/case-flow-human-steps/specs/status-transition-engine/spec.md
	 */
	public function lookupStatusIdByName(string $caseTypeId, string $statusName): string {
		...
		$statusTypeSchema = $this->settingsService->getConfigValue(key: 'status_type_schema');
		if ($register === '' || $caseTypeSchema === '' || $statusTypeSchema === '') {
			return '';
		}
		...
		// Both spellings, exactly as assertStatusBelongsToCaseType() reads them.
		// One reader accepting a key the other does not is how a case type ends
		...
		foreach ($statuses as $entry) {
			...
			// An embedded entry carries its own name; a bare uuid has to be
			// resolved. Reading the embedded name first avoids a lookup per
			// status on the common path.
			if ($name === '') {
				$name = $this->lookupStatusName(statusTypeId: $id);
			}
