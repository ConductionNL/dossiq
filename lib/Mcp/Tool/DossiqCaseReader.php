...
/**
 * Dossiq MCP case reader.
 *
 * Every OpenRegister read the Dossiq MCP tools perform, plus the shape
 * normalisation that turns whatever ObjectService returns (entity, array,
 * null) into a plain associative array. Split out of DossiqToolProvider so
...
/**
 * Reads Dossiq cases and their transition history for the MCP tools.
 *
 * @spec openspec/specs/mcp-integration/spec.md
 */
class DossiqCaseReader {
	use SearchesObjects;
	...
	public function resolveCaseStore(): array {
		...
		if ($register === '' || $caseSchema === '') {
			return [
				'ok' => false,
				'code' => 'not_configured',
				'message' => 'The Dossiq case schema is not configured.',
			];
		}
	...
	public function findCases(array $store, array $filters, int $limit): ?array {
		try {
			...
			$this->logger->error(
				'Dossiq MCP: listProcesses search failed',
	...
	public function findCase(array $store, string $caseId): ?array {
		try {
			...
			$this->logger->error(
				'Dossiq MCP: getProcessDetails findObject failed',
	...
	public function loadHistory(array $store, string $caseUuid): array {
		...
		try {
			...
			$this->logger->error(
				'Dossiq MCP: loadHistory search failed',
