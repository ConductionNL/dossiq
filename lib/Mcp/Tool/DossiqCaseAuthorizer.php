...
/**
 * Dossiq MCP case authorizer.
 *
 * The per-object read authorisation the Dossiq MCP tools apply before any
 * case data leaves the app (OWASP A01:2021 / ADR-005). Split out of
 * DossiqToolProvider so the rule — admin, assignee, or role-holder — has one
...
/**
 * Decides whether the calling user may read a Dossiq case over MCP.
 *
 * @spec openspec/specs/mcp-integration/spec.md
 */
class DossiqCaseAuthorizer {
	use SearchesObjects;

	/**
	 * Dedicated procest-admin group id (mirrors TransitionAuthorizer).
	 *
	 * FROZEN at the old app-id prefix. This is a NEXTCLOUD GROUP ID, and
	 * OpenRegister provisions declared groups create-only: renaming it would
	 * create a new, EMPTY group and orphan the populated one, so every
	 * administrator would silently lose MCP read access to cases they are
	 * entitled to. Must stay in step with TransitionAuthorizer::ADMIN_GROUP_ID.
	 *
	 * @var string
	 */
	 ...
	 * Constructor.
	 *
	 * @param SettingsService $settingsService The Dossiq settings service (OpenRegister bridge + config).
	...
	private function hasRoleOnCase(string $caseUuid, string $userId): bool {
		...
		try {
			...
			$this->logger->error(
				'Dossiq MCP: hasRoleOnCase search failed',
	...
	/**
	 * Check whether the user is a Dossiq or Nextcloud system administrator.
	 *
	 * @param string $userId The Nextcloud user id.
	 *
