...
class SentimentAnalysisJob extends TimedJob
{
    ...
    protected function run($argument): void
    {
        ...
        try {
            $contacts = $objectService->findObjects($register, $contactmomentSchema, [], [], 200);
        } catch (Throwable $e) {
            $this->logger->error(
    ...
    private function processContact($objectService, string $register, string $sentimentSchema, array $contact, array $triggerWords): void
    {
        ...
        $existing = $objectService->findObjects($register, $sentimentSchema, ['contactmomentId' => $contactId], [], 1);
