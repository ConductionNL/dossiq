 ...
 * are declared in `src/manifest.json` (`observability.health`) and executed
 ...
 * (`#[PublicPage]`) is re-declared here so it is visible to NC's middleware
 * and to the route-auth gate, then delegates to the engine.
 *
 * The parent class is only autoloaded when NC instantiates this controller on
 * a request to `/api/health` — never at `Application::register()` — so a
 * disabled or absent OpenRegister does not fatal Nextcloud bootstrap; the
 * first health request degrades to a 5xx instead.
 *
 * @category Controller
...
class HealthController extends GenericHealthController {
	 ...
	 * manifest + resolves dossiq's version.
	 *
