...
/**
 * Ratings in the order they are counted and rendered. The audit emits no
 * other value; an unknown rating is counted under `unknown` rather than
 ...
 * the same shape `docs/features.json` uses for `title`/`title_nl`. Dutch wins
 * only for a Dutch locale AND only when the Dutch string is actually present,
 * so a half-translated file degrades to English rather than to a blank cell.
 *
...
/**
 * Format the comparison date for the reader.
 *
 ...
 * through `Intl` so a Dutch reader gets `7 september 2026`, and falls back to
