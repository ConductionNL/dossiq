...
<!--
	The case's banner strips, in one grid row instead of four.

	Favourite, New since you last looked, What this status asks for and
	Attention were four `gridWidth: 12, gridHeight: 1` widgets stacked down the
	page. Three of the four render conditionally — `CaseUnreadPanel`,
	...
	content. One row holding all four is the first; `sizeToContent` on that row
	(nextcloud-vue CnDashboardGrid) is the second, and this widget uses both.

	The order is the order the four rows had, which is the order they are read
	in: what I marked, what changed, what the status wants, what is wrong.

	Each panel keeps its own `v-if`, its own fetch and its own tests. This adds
	no conditions of its own: it is a container, and the day a fifth strip
	appears it belongs here rather than in a fifth row.
