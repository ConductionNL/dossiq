 ...
 *
 * Shared config for the compact "signalling" dashboard list widgets
 * (open cases, overdue, my tasks, deadline alerts, task reminders, stalled
 * cases). Each widget self-fetches and shapes its rows to `{ id, mainText,
 * subText, targetUrl }`, then renders the universal `<CnDataTable>` headerless
 * with these columns — a bold name and a muted, right-aligned trailing status,
 * matching the compact list look shared with scholiq.
 */
 ...
 * history router and when the widget runs standalone on the Nextcloud
 * Dashboard (where no vue-router is present).
 *
