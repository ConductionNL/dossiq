		...
		<div v-if="consultations.length > 0" class="consultation-panel__list">
			<div
				...
				<div class="consultation-panel__item-header">
					<span class="consultation-panel__department">{{ cons.adviesAuthority }}</span>
					<span
				...
				<div class="consultation-panel__meta">
					<span>{{ t('procest', 'Deadline: {date}', { date: formatDate(cons.latestResponseDate) }) }}</span>
					<span v-if="cons.applicant">{{ t('procest', 'by {user}', { user: cons.applicant }) }}</span>
				</div>
...
export default {
	...
	methods: {
		...
		conditions(cons) {
			...
			try {
				const parsed = typeof cons.terms === 'string'
