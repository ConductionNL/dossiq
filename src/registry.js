...
const registry = {
	...
	CaseSplitDialog: {
		...
		_note: "Dividing one case in two (REQ-SPL-01). The inverse of CaseMergeDialog and a different act from CaseCopyDialog: a copy opens a second case holding everything, so both claim the same document, and a split MOVES what is ticked and leaves a reference behind. Nothing is ticked for the handler, because which document belongs to which half is a judgement about content and a default selection is a judgement that gets confirmed. A party ticked as moving AND as on both halves is two contradictory instructions, so the button refuses rather than the server picking one. The refusals are the server's and shown verbatim: a case type that forbids dividing documents and a task the engine cannot move are two answers with two ways out. The header action that opens it is hidden on a CLOSED case, for the same reason the merge is: a closed case is a record of what was decided, and dividing one after the fact would change what a decision was about. That reasoning lives here and not in the manifest because the v2 manifest schema rejects `_note` on a header action, which is why `case-merge` fails the validator today.",
	},
